...
function publiclyAddressed(recipients) {
	return activitypub._constants.acceptablePublicAddresses.some((address) =>
...
inbox.create = async (req) => {
	...
	const isPublic = publiclyAddressed([
		...(object.to || []),
		...(object.cc || []),
	]);
	if (!isPublic) {
		...
		if (context) {
			...
			if (context === target && attributedTo === actor) {
				activitypub.helpers.log(
					`[activitypub/inbox/add] Associating pid ${pid} with new context ${target}`,
				);
...
inbox.remove = async (req) => {
	...
	const isContext = activitypub._constants.acceptable.contextTypes.has(
		object.type,
	);
	if (!isContext) {
	...
	activitypub.helpers.log(
		`[activitypub/inbox/remove] Removing topic ${tid} from ${cid}`,
	);
...
inbox.move = async (req) => {
	...
	activitypub.helpers.log(
		`[activitypub/inbox/remove] Moving topic ${tid} from ${fromCid} to ${toCid}`,
	);
	await topics.tools.move(tid, {
		cid: toCid,
		uid: 'system',
			...
			try {
				switch (true) {
					case isNote: {
						...
						postData.tags = await activitypub.notes._normalizeTags(
							postData._activitypub.tag,
							postData.cid,
						);
					...
					case isMessage: {
						const { roomId, deleted } = await messaging.getMessageFields(
							object.id,
							['roomId', 'deleted'],
						);
...
inbox.delete = async (req) => {
	...
	switch (true) {
		...
		case !!isContext: {
			...
			if (!exists) {
				activitypub.helpers.log(
					`[activitypub/inbox.delete] Context main pid (${pid}) not found locally. Doing nothing.`,
				);
				return;
			}
			const { tid, uid } = await posts.getPostFields(pid, ['tid', 'uid']);
			activitypub.helpers.log(
				`[activitypub/inbox.delete] Deleting tid ${tid}.`,
			);
		...
		default: {
			activitypub.helpers.log(
				`[activitypub/inbox.delete] Object (${id}) does not exist locally. Doing nothing.`,
			);
			break;
		}
	}
...
inbox.like = async (req) => {
	...
	if (!allowed) {
		activitypub.helpers.log(
			`[activitypub/inbox.like] ${id} not allowed to be upvoted.`,
		);
		return reject('Like', object, actor);
	}
...
inbox.dislike = async (req) => {
	...
	if (!allowed) {
		activitypub.helpers.log(
			`[activitypub/inbox.like] ${id} not allowed to be downvoted.`,
		);
		return reject('Dislike', object, actor);
	}
...
inbox.announce = async (req) => {
	...
	timestamp =
		timestamp.toString() !== 'Invalid Date' ? timestamp.getTime() : Date.now();
		...
		case activitypub._constants.acceptedPostTypes.includes(object.type): {
			if (String(object.id).startsWith(nconf.get('url'))) {
				...
				if (!fromRelay && !cid && !syncedCids.length) {
					...
					if (!followers) {
						winston.verbose(
							`[activitypub/inbox.announce] Rejecting ${object.id} via ${actor} due to no followers`,
						);
						reject('Announce', object, actor);
						return;
					}
			...
			if (!cid) {
				// Topic events from actors followed by users only
...
inbox.accept = async (req) => {
	...
	if (type === 'Follow') {
		if (localType === 'user') {
			...
			const timestamp = await db.sortedSetScore(
				`followRequests:uid.${id}`,
				actor,
			);
			await Promise.all([
			...
			const followingRemoteCount = await db.sortedSetCard(
				`followingRemote:${id}`,
			);
...
inbox.undo = async (req) => {
	...
	winston.verbose(
		`[activitypub/inbox/undo] ${type} ${localType && id ? `${localType} ${id}` : object.object} via ${actor}`,
	);
		...
		case 'Follow': {
			switch (localType) {
				case 'user': {
					...
					const followerRemoteCount = await db.sortedSetCard(
						`followerRemote:${id}`,
					);
...
inbox.flag = async (req) => {
	...
	await Promise.all(
		objects.map(async (subject, index) => {
			const { type, id } = await activitypub.helpers.resolveObjects(subject.id);
			try {
				await flags.create(
					activitypub.helpers.mapToLocalType(type),
					id,
					actor,
					content,
				);
