...
ActivityPub.get = async (type, id, uri, options) => {
	...
	if (!allowed) {
		ActivityPub.helpers.log(
			`[activitypub/get] Not retrieving ${uri}, domain is blocked.`,
		);
...
async function retryFailedMessages() {
	...
	const queuedData = await db.getObjects(
		queueIds.map((id) => `ap:retry:queue:${id}`),
	);
	...
	const oneMinute = 1000 * 60;
	await Promise.all(
		queuedData.map(async (data, index) => {
...
ActivityPub.checkHeader = async (url, timeout) => {
	...
	try {
		...
		ActivityPub.helpers.log(
			`[activitypub/checkHeader] Failed on ${url}: ${e.message}`,
		);
		return false;
	}
};
