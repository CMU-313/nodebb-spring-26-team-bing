...
Actors.assertGroup = async (ids, options = {}) => {
	...
	// Privilege mask
	const [masksAdd, masksRemove] = categoryObjs.reduce(
		([add, remove], category) => {
			(category?._activitypub?.postingRestrictedToMods ? add : remove).push(
				`cid:${category.cid}:privilegeMask`,
			);
			return [add, remove];
		},
		[[], []],
...
Actors.getLocalFollowCounts = async (actors) => {
	...
	const actorToCounts = _.zipObject(
		validActors,
	...
	await batch.processArray(
		...
		async (ids) => {
			...
			if (cleanupUids.length) {
				...
				winston.info(
					`[actors/prune] Cleaned up ${cleanupUids.length} remote users that were not found in the database.`,
				);
			}
