...
Feps.announce = async function announce(id, activity) {
	...
	if (!shouldAnnounce) {
		// inverse conditionals can kiss my ass.
		return;
	}
	...
	const followers = localCid
		? await activitypub.notes.getCategoryFollowers(cid)
		: [cid];
	...
	if (activity.type === 'Create') {
		...
		if (isMain) {
			activitypub.helpers.log(
				`[activitypub/inbox.announce(1b12)] Announcing plain object (${activity.id}) to followers of cid ${cid} and ${relays.length} relays`,
			);
	...
	activitypub.helpers.log(
		`[activitypub/inbox.announce(1b12)] Announcing ${activity.type} (${activity.id}) to followers of cid ${cid} and ${relays.length} relays`,
	);
	await activitypub.send('cid', localCid ? cid : 0, targets, {
