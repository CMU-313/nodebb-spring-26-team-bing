...
Notes._normalizeTags = async (tag, cid) => {
	...
	if (!Array.isArray(tags)) {
		// the "|| []" should handle null/undefined values... #famouslastwords
		tags = [tags];
	}
...
Notes.assert = async (uid, input, options = { skipChecks: false }) => {
	...
	if (!lockStatus) {
		...
		winston.warn(
			`[activitypub/notes.assert] Unable to acquire lock, skipping processing of ${id}`,
		);
		return null;
	}
	...
	try {
		...
		if (hasTid) {
			...
			try {
				...
				remoteCid = Array.from(set)
					.filter((id, idx) => {
						...
						return (
							assertedGroups[idx] &&
							(explicitAudience || cidHostname === hostname)
						);
					})
					.shift();
			} catch (e) {
				// noop
				winston.error(
					'[activitypub/notes.assert] Could not parse URL of mainPid',
					e.stack,
				);
			}
			...
			if (!title) {
				...
				prettified = prettified
					.split('\n')
					.filter((line) => !line.startsWith('<p class="quote-inline"'))
					.join('\n');
		...
		winston.warn(
			`[activitypub/notes.assert] Could not assert ${id} (${e.message}).`,
		);
		return null;
	} finally {
			...
			switch (type) {
				case 'hashtag': {
					if (tags.includes(value.toLowerCase())) {
						activitypub.helpers.log(
							`[activitypub]   - Rule match: #${value}; cid: ${target}`,
						);
						return target;
					}
					break;
				...
				case 'user': {
					if (post.uid === value) {
						activitypub.helpers.log(
							`[activitypub]   - Rule match: user ${value}; cid: ${target}`,
						);
						return target;
					}
				}
...
Notes.getParentChain = async (uid, input) => {
	...
	const traverse = async (uid, id) => {
		...
		if (postData) {
			...
			let object =
				!activitypub.helpers.isUri(input) && input.id === id
					? input
					: undefined;
			try {
...
Notes.getCategoryFollowers = async (cid) => {
	...
	let uids = await db.getSortedSetRangeByScore(
...
Notes.prune = async () => {
	...
	const hasLocalVoter = await Promise.all(
		tids.map(async (tid) => {
			...
			await Promise.all(
				pids.map(async (pid) => {
					const [upvoters, downvoters] = await db.getSetsMembers([
						`pid:${pid}:upvote`,
						`pid:${pid}:downvote`,
					]);
