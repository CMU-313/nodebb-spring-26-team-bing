...
privsTopics.get = async function (tid, uid) {
	...
	const [userPrivileges, isAdministrator, isModerator, disabled, topicTools] =
		await Promise.all([
	...
	const deletable =
		(privData["topics:delete"] && (isOwner || isModerator)) || isAdministrator;
	const mayReply = privsTopics.canViewDeletedScheduled(
		topicData,
		{},
		false,
		privData["topics:schedule"],
	);
	const hasTools = topicTools.tools.length > 0;

	return await plugins.hooks.fire("filter:privileges.topics.get", {
		"topics:reply":
			(privData["topics:reply"] &&
				((!topicData.locked && mayReply) || isModerator)) ||
			isAdministrator,
		"topics:read": privData["topics:read"] || isAdministrator,
		"topics:schedule": privData["topics:schedule"] || isAdministrator,
		"topics:tag": privData["topics:tag"] || isAdministrator,
		"topics:delete":
			(privData["topics:delete"] && (isOwner || isModerator)) ||
			isAdministrator,
		"posts:edit":
			(privData["posts:edit"] && (!topicData.locked || isModerator)) ||
			isAdministrator,
		...
		"posts:delete":
			(privData["posts:delete"] && (!topicData.locked || isModerator)) ||
			isAdministrator,
...
privsTopics.filterUids = async function (privilege, tid, uids) {
	...
	if (topicData.scheduled) {
		const canViewScheduled = await helpers.isUsersAllowedTo(
			"topics:schedule",
			uids,
			topicData.cid,
		);
	...
	return uids.filter(
		(uid, index) =>
			!disabled &&
			((allowedTo[index] && (topicData.scheduled || !topicData.deleted)) ||
				isAdmins[index]),
	);
};
...
privsTopics.canDelete = async function (tid, uid) {
	...
	return (
		allowedTo[0] &&
		((isOwner && (deleterUid === 0 || deleterUid === topicData.uid)) ||
			isModerator)
	);
};
...
privsTopics.canViewDeletedScheduled = function (
	topic,
	privileges = {},
	viewDeleted = false,
	viewScheduled = false,
	...
	const { view_deleted = viewDeleted, view_scheduled = viewScheduled } =
		privileges;
