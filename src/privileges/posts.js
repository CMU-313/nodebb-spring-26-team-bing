	...
	const privileges = cids.map((cid, i) => {
		...
		const editable =
			(privData["posts:edit"][cid] &&
				(results.isOwner[i] || results.isModerator[i])) ||
			results.isAdmin;
		const viewDeletedPosts =
			...
			results.isAdmin;
		const viewHistory =
			results.isOwner[i] || privData["posts:history"][cid] || results.isAdmin;
...
privsPosts.canDelete = async function (pid, uid) {
	...
	const flag =
		...
		((results.isOwner && (deleterUid === 0 || deleterUid === postData.uid)) ||
			results.isMod);
...
privsPosts.canPurge = async function (pid, uid) {
	...
	return (
		(results.purge && (results.owner || results.isModerator)) || results.isAdmin
	);
};
