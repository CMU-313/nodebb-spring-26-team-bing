...
module.exports = function (Posts) {
	...
	Posts.uploads.dissociate = async function (pid, filePaths) {
		...
		currentUploads = currentUploads.filter(
			(upload) => !filePaths.includes(upload),
		);
		...
		if (!meta.config.preserveOrphanedUploads) {
			const deletePaths = (
				await Promise.all(
					filePaths.map(async (filePath) =>
						(await Posts.uploads.isOrphan(filePath)) ? filePath : false,
					),
				)
			).filter(Boolean);
	...
	Posts.uploads.deleteFromDisk = async (filePaths) => {
		if (typeof filePaths === 'string') {
			...
			throw new Error(
				`[[error:wrong-parameter-type, filePaths, ${typeof filePaths}, array]]`,
			);
		}
		...
		await Promise.all(
			filePaths.map(async (fileName) => {
				try {
					...
					winston.error(
						`[posts/uploads] Error while saving post upload sizes (${fileName}): ${err.message}`,
					);
				}
			}),
		);
