...
module.exports = function (User) {
	...
	function getSetting(settings, key, defaultValue) {
		if (settings[key] || settings[key] === 0) {
			...
			activitypub.helpers.isUri(settings.uid) &&
			remoteDefaultSettings[key]
		) {
	...
	User.saveSettings = async function (uid, data) {
		...
		if (
			...
			throw new Error(
				`[[error:invalid-pagination-value, 2, ${maxPostsPerPage}]]`,
			);
		}
		...
		if (
			...
			throw new Error(
				`[[error:invalid-pagination-value, 2, ${maxTopicsPerPage}]]`,
			);
		}
