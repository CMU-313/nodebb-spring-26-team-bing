			...
			async (pids) => {
				...
				for (const key of keys) {
					...
					uploads = uploads.filter(
						(upload) =>
							upload && upload.value && !upload.value.startsWith("files/"),
					);
					...
					const newHashes = uploads.map((upload) =>
						md5(`files/${upload.value}`),
					);
					...
					const oldData = await db.getObjects(
						hashes.map((hash) => `upload:${hash}`),
					);
					const bulkSet = [];
					...
					await Promise.all(
						hashes.map((hash, idx) =>
							db.rename(`upload:${hash}:pids`, `upload:${newHashes[idx]}:pids`),
						),
					);
				}
			},
