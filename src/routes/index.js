...
_mounts.topic = (app, name, middleware, controllers) => {
	setupPageRoute(
		app,
		`/${name}/:topic_id/:slug/:post_index?`,
		[],
		controllers.topics.get,
	);
...
_mounts.tags = (app, name, middleware, controllers) => {
	setupPageRoute(
		app,
		`/${name}/:tag`,
		[middleware.privateTagListing],
		controllers.tags.getTag,
	);
	setupPageRoute(
		app,
		`/${name}`,
...
_mounts.category = (app, name, middleware, controllers) => {
	setupPageRoute(
		app,
		`/${name}/:category_id/:slug/:topic_index`,
		[],
...
_mounts.groups = (app, name, middleware, controllers) => {
	...
	setupPageRoute(
		...
		`/${name}/:slug`,
		middlewares,
		controllers.groups.details,
	);
	setupPageRoute(
		app,
		`/${name}/:slug/members`,
		middlewares,
		controllers.groups.members,
	);
};
...
function addCoreRoutes(app, router, middleware, mounts) {
	...
	if (
		path.resolve(__dirname, "../../public/uploads") !== nconf.get("upload_path")
	) {
		statics.unshift({
