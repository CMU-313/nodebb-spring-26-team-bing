...
module.exports = function (Posts) {
  ...
  async function deleteOrRestore(type, pid, uid) {
    ...
    await Promise.all([topics.updateLastPostTimeFromLastPid(postData.tid), topics.updateTeaser(postData.tid), isDeleting ? activitypub.notes.delete(pid) : null, isDeleting ? db.sortedSetRemove(`cid:${topicData.cid}:pids`, pid) : db.sortedSetAdd(`cid:${topicData.cid}:pids`, postData.timestamp, pid)]);
  ...
  Posts.purge = async function (pids, uid) {
    ...
    await Promise.all([deleteFromTopicUserNotification(postData), deleteFromCategoryRecentPosts(postData), deleteFromUsersBookmarks(pids), deleteFromUsersVotes(pids), deleteFromReplies(postData), deleteFromGroups(pids), deleteDiffs(pids), deleteFromUploads(pids), db.sortedSetsRemove(['posts:pid', 'posts:votes', 'posts:flagged'], pids), Posts.attachments.empty(pids), activitypub.notes.delete(pids), db.deleteAll(pids.map(pid => `pid:${pid}:editors`))]);
  ...
  async function deleteFromTopicUserNotification(postData) {
    ...
    await Promise.all([db.incrObjectFieldByBulk(incrObjectBulk), db.sortedSetAddBulk(topicPostCountTasks), ...topicTasks, user.updatePostCount(_.uniq(postData.map(p => p.uid))), notifications.rescind(...postData.map(p => `new_post:tid:${p.tid}:pid:${p.pid}:uid:${p.uid}`))]);
    ...
    await db.setObjectBulk(tids.map((tid, idx) => [`topic:${tid}`, {
  ...
  async function deleteFromUsersVotes(pids) {
    const [upvoters, downvoters] = await Promise.all([db.getSetsMembers(pids.map(pid => `pid:${pid}:upvote`)), db.getSetsMembers(pids.map(pid => `pid:${pid}:downvote`))]);
    const bulkRemove = [];
    pids.forEach((pid, index) => {
    ...
    await Promise.all([db.sortedSetRemoveBulk(bulkRemove), db.deleteAll([...pids.map(pid => `pid:${pid}:upvote`), ...pids.map(pid => `pid:${pid}:downvote`)])]);
  }
