...
categoriesAPI.update = async function (caller, data) {
  ...
  activitypub.out.update.category(cid);
};
categoriesAPI.delete = async function (caller, {
  cid
}) {
  await hasAdminPrivilege(caller.uid);
...
categoriesAPI.getTopicCount = async (caller, {
  cid
}) => {
...
categoriesAPI.getPosts = async (caller, {
  cid
}) => await categories.getRecentReplies(cid, caller.uid, 0, 4);
categoriesAPI.getChildren = async (caller, {
  cid,
  start
}) => {
  if (!start || start < 0) {
...
categoriesAPI.getTopics = async (caller, data) => {
  ...
  const [userPrivileges, settings, targetUid] = await Promise.all([privileges.categories.get(data.cid, caller.uid), user.getSettings(caller.uid), user.getUidByUserslug(data.query.author)]);
...
categoriesAPI.setWatchState = async (caller, {
  cid,
  state,
  uid
}) => {
...
categoriesAPI.getPrivileges = async (caller, {
  cid
}) => {
...
categoriesAPI.setModerator = async (caller, {
  cid,
  member,
  set
}) => {
