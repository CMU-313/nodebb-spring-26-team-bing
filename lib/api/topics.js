...
topicsAPI._checkThumbPrivileges = async function ({
  tid,
  uid
}) {
...
topicsAPI.get = async function (caller, data) {
  ...
  if (!topic || !userPrivileges.read || !userPrivileges['topics:read'] || !(await topics.canViewTopic(topic, caller.uid)) || !privileges.topics.canViewDeletedScheduled(topic, userPrivileges)) {
    return null;
  }
  return topic;
};
topicsAPI.create = async function (caller, data) {
  ...
  socketHelpers.emitToUids('event:new_post', {
...
topicsAPI.pin = async function (caller, {
  tids,
  expiry
}) {
...
topicsAPI.updateTags = async (caller, {
  tid,
  tags
}) => {
...
topicsAPI.addTags = async (caller, {
  tid,
  tags
}) => {
  if (!(await privileges.topics.canEdit(tid, caller.uid))) {
...
topicsAPI.deleteTags = async (caller, {
  tid
}) => {
  if (!(await privileges.topics.canEdit(tid, caller.uid))) {
    throw new Error('[[error:no-privileges]]');
...
topicsAPI.getThumbs = async (caller, {
  tid,
  thumbsOnly
}) => {
  if (isFinite(tid)) {
    const [exists, canRead] = await Promise.all([topics.exists(tid), privileges.topics.can('topics:read', tid, caller.uid)]);
    if (!exists) {
...
topicsAPI.deleteThumb = async (caller, {
  tid,
  path
}) => {
  await topicsAPI._checkThumbPrivileges({
    tid: tid,
    uid: caller.uid
  });
  await topics.thumbs.delete(tid, path);
};
topicsAPI.reorderThumbs = async (caller, {
  tid,
  path,
  order
}) => {
...
topicsAPI.getEvents = async (caller, {
  tid
}) => {
...
topicsAPI.deleteEvent = async (caller, {
  tid,
  eventId
}) => {
...
topicsAPI.markRead = async (caller, {
  tid
}) => {
...
topicsAPI.markUnread = async (caller, {
  tid
}) => {
  if (!tid || caller.uid <= 0) {
    throw new Error('[[error:invalid-data]]');
    ...
    await Promise.all(tids.map(async tid => {
      ...
      if (!topicData.deleted) {
        ...
        if (utils.isNumber(cid) && parseInt(cid, 10) === -1) {
          ...
          activitypub.out.delete.note(caller.uid, topicData.mainPid);
        } else {
