...
async function rateLimitExceeded(caller, field) {
  const session = caller.request ? caller.request.session : caller.session;
  const now = Date.now();
  const [isPrivileged, reputation] = await Promise.all([user.isPrivileged(caller.uid), user.getUserField(caller.uid, 'reputation')]);
...
chatsAPI.create = async function (caller, data) {
  ...
  data.notificationSetting = isPublic ? messaging.notificationSettings.ATMENTION : messaging.notificationSettings.ALLMESSAGES;
...
chatsAPI.sortPublicRooms = async (caller, {
  roomIds,
  scores
}) => {
...
chatsAPI.get = async (caller, {
  uid,
  roomId
}) => await messaging.loadRoom(caller.uid, {
  uid,
  roomId
});
chatsAPI.post = async (caller, data) => {
  ...
  ({
    data
  } = await plugins.hooks.fire('filter:messaging.send', {
    data,
    uid: caller.uid
  }));
...
chatsAPI.mark = async (caller, data) => {
  ...
  if (state) {
    ...
    socketHelpers.emitToUids('event:chats.markedAsRead', {
      roomId: roomId
    }, [caller.uid]);
  ...
  socketHelpers.emitToUids('event:chats.mark', {
    roomId,
    state
  }, [caller.uid]);
  messaging.pushUnreadCount(caller.uid);
};
chatsAPI.watch = async (caller, {
  roomId,
  state
}) => {
...
chatsAPI.toggleTyping = async (caller, {
  roomId,
  typing
}) => {
  ...
  const [isInRoom, username] = await Promise.all([messaging.isUserInRoom(caller.uid, roomId), user.getUserField(caller.uid, 'username')]);
  if (!isInRoom) {
    throw new Error('[[error:no-privileges]]');
  }
...
chatsAPI.users = async (caller, data) => {
  ...
  const [isOwner, isUserInRoom, users, isAdmin, onlineUids] = await Promise.all([messaging.isRoomOwner(caller.uid, data.roomId), messaging.isUserInRoom(caller.uid, data.roomId), messaging.getUsersInRoomFromSet(`chat:room:${data.roomId}:uids:online`, data.roomId, start, stop, true), user.isAdministrator(caller.uid), io.getUidsInRoom(`chat_room_${data.roomId}`)]);
  if (!isUserInRoom) {
    throw new Error('[[error:no-privileges]]');
  }
  users.forEach(user => {
...
chatsAPI.toggleOwner = async (caller, {
  roomId,
  uid,
  state
}) => {
  const [isAdmin, inRoom, isRoomOwner] = await Promise.all([user.isAdministrator(caller.uid), messaging.isUserInRoom(caller.uid, roomId), messaging.isRoomOwner(caller.uid, roomId)]);
  if (!isAdmin && (!inRoom || !isRoomOwner)) {
    throw new Error('[[error:no-privileges]]');
  }
  return await messaging.toggleOwner(uid, roomId, state);
};
chatsAPI.listMessages = async (caller, {
  uid = caller.uid,
  roomId,
  start = 0,
  direction = null
...
chatsAPI.getPinnedMessages = async (caller, {
  start,
  roomId
}) => {
...
chatsAPI.getMessage = async (caller, {
  mid,
  roomId
} = {}) => {
  if (!mid || !roomId) {
...
chatsAPI.getRawMessage = async (caller, {
  mid,
  roomId
} = {}) => {
  if (!mid || !roomId) {
  ...
  const [isAdmin, canViewMessage, inRoom] = await Promise.all([user.isAdministrator(caller.uid), messaging.canViewMessage(mid, roomId, caller.uid), messaging.isUserInRoom(caller.uid, roomId)]);
...
chatsAPI.getIpAddress = async (caller, {
  mid
}) => {
...
chatsAPI.editMessage = async (caller, {
  mid,
  roomId,
  message
}) => {
...
chatsAPI.unpinMessage = async (caller, {
  roomId,
  mid
}) => {
  await messaging.canPin(roomId, caller.uid);
