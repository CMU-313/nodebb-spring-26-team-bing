...
utils.tokens.get = async tokens => {
  ...
  let [tokenObjs, lastSeen] = await Promise.all([db.getObjects(tokens.map(t => `token:${t}`)), utils.tokens.getLastSeen(tokens)]);
...
utils.tokens.add = async ({
  ...
  await Promise.all([db.setObject(`token:${token}`, {
    uid,
    description,
    timestamp
  }), db.sortedSetAdd(`tokens:createtime`, timestamp, token), db.sortedSetAdd(`tokens:uid`, uid, token)]);
  return token;
};
utils.tokens.update = async (token, {
  uid,
  description
}) => {
  if (!srcUtils.isNumber(uid)) {
...
utils.tokens.roll = async token => {
  ...
  const updates = [db.rename(`token:${token}`, `token:${newToken}`), db.sortedSetsRemove([`tokens:createtime`, `tokens:uid`, `tokens:lastSeen`], token), db.sortedSetAdd(`tokens:createtime`, createTime, newToken), db.sortedSetAdd(`tokens:uid`, uid, newToken)];
  if (lastSeen) {
...
utils.tokens.delete = async token => {
  await Promise.all([db.delete(`token:${token}`), db.sortedSetsRemove([`tokens:createtime`, `tokens:uid`, `tokens:lastSeen`], token)]);
};
