...
groupsAPI.leave = async function (caller, data) {
  ...
  const [groupData, isCallerOwner, userExists, isMember] = await Promise.all([groups.getGroupData(groupName), isOwner(caller, groupName, false), user.exists(data.uid), groups.isMember(data.uid, groupName)]);
  if (!isMember) {
  ...
  const {
    displayname
  } = await user.getUserFields(data.uid, ['username']);
...
groupsAPI.getPending = async (caller, {
  slug
}) => {
...
groupsAPI.getInvites = async (caller, {
  slug
}) => {
  const groupName = await groups.getGroupNameByGroupSlug(slug);
  await isOwner(caller, groupName);
...
groupsAPI.issueInvite = async (caller, {
  slug,
  uid
}) => {
  const groupName = await groups.getGroupNameByGroupSlug(slug);
...
groupsAPI.acceptInvite = async (caller, {
  slug,
  uid
}) => {
  const groupName = await groups.getGroupNameByGroupSlug(slug);
...
groupsAPI.rejectInvite = async (caller, {
  slug,
  uid
}) => {
  const groupName = await groups.getGroupNameByGroupSlug(slug);
