...
postsAPI.getIndex = async (caller, {
  pid,
  sort
}) => {
  const [tid, postData, isAdmin] = await Promise.all([posts.getPostField(pid, 'tid'), posts.getPostFields(pid, ['uid', 'visibilityMode', 'anonymous']), postVisibility.isViewerAdmin(caller.uid)]);
...
postsAPI.getSummary = async (caller, {
  pid
}) => {
...
postsAPI.getRaw = async (caller, {
  ...
  const [postData, isAdmin] = await Promise.all([posts.getPostFields(pid, ['content', 'deleted', 'uid', 'visibilityMode', 'anonymous']), postVisibility.isViewerAdmin(caller.uid)]);
  if (!postData) {
    return null;
  }
  if (!postVisibility.canViewPost(postData, caller.uid, isAdmin)) {
...
async function deleteOrRestore(caller, data, params) {
  ...
  const [postData, {
    isMain,
    isLast
  }] = await Promise.all([posts.tools[params.command](caller.uid, data.pid), isMainAndLastPost(data.pid), activitypub.out.delete.note(caller.uid, data.pid)]);
  if (isMain && isLast) {
...
postsAPI.purge = async function (caller, data) {
  ...
  const [exists, {
    isMain,
    isLast
  }] = await Promise.all([posts.exists(data.pid), isMainAndLastPost(data.pid)]);
  if (!exists) {
  ...
  await Promise.all([posts.purge(data.pid, caller.uid), activitypub.out.delete.note(caller.uid, data.pid)]);
...
postsAPI.move = async function (caller, data) {
  ...
  const [postDeleted, topicDeleted] = await Promise.all([posts.getPostField(data.pid, 'deleted'), topics.getTopicField(data.tid, 'deleted'), await events.log({
    type: `post-move`,
    uid: caller.uid,
    ip: caller.ip,
    pid: data.pid,
  ...
  if (!postDeleted && !topicDeleted) {
    ...
    if (utils.isNumber(data.pid)) {
      const {
        activity
      } = await activitypub.mocks.activities.create(data.pid, caller.uid);
...
postsAPI.getVoters = async function (caller, data) {
  ...
  const [canSeeUpvotes, canSeeDownvotes] = await Promise.all([canSeeVotes(caller.uid, cid, 'upvoteVisibility'), canSeeVotes(caller.uid, cid, 'downvoteVisibility')]);
  ...
  const [upvoteUids, downvoteUids] = await Promise.all([showUpvotes ? db.getSetMembers(`pid:${data.pid}:upvote`) : [], showDownvotes ? db.getSetMembers(`pid:${data.pid}:downvote`) : []]);
...
postsAPI.getAnnouncers = async (caller, data) => {
  ...
  const announcers = await notes.announce.list({
    pid
  });
...
async function canSeeVotes(uid, cids, type) {
  ...
  const [canRead, isAdmin, isMod] = await Promise.all([privileges.categories.isUserAllowedTo('topics:read', uniqCids, uid), privileges.users.isAdministrator(uid), privileges.users.isModerator(uid, cids)]);
  const cidToAllowed = _.zipObject(uniqCids, canRead);
  const checks = cids.map((cid, index) => isAdmin || isMod[index] || cidToAllowed[cid] && (meta.config[type] === 'all' || meta.config[type] === 'loggedin' && parseInt(uid, 10) > 0));
...
postsAPI.getDiffs = async (caller, data) => {
  ...
  const [timestamps, post, diffs] = await Promise.all([posts.diffs.list(data.pid), posts.getPostFields(data.pid, ['timestamp', 'uid']), posts.diffs.get(data.pid)]);
...
postsAPI.deleteDiff = async (caller, {
  pid,
  timestamp
}) => {
  const cid = await posts.getCidByPid(pid);
  const [isAdmin, isModerator] = await Promise.all([privileges.users.isAdministrator(caller.uid), privileges.users.isModerator(caller.uid, cid)]);
  if (!(isAdmin || isModerator)) {
    throw new Error('[[error:no-privileges]]');
  }
  await posts.diffs.delete(pid, timestamp, caller.uid);
};
postsAPI.getReplies = async (caller, {
  pid
}) => {
  ...
  const [canRead, parentPost, isAdmin] = await Promise.all([privileges.posts.can('topics:read', pid, caller.uid), posts.getPostFields(pid, ['uid', 'visibilityMode', 'anonymous']), postVisibility.isViewerAdmin(caller.uid)]);
  if (!canRead) {
    return null;
  }
  ...
  const {
    topicPostSort
  } = await user.getSettings(uid);
  const pids = await posts.getPidsFromSet(`pid:${pid}:replies`, 0, -1, topicPostSort === 'newest_to_oldest');
  let [postData, postPrivileges] = await Promise.all([posts.getPostsByPids(pids, uid), privileges.posts.get(pids, uid)]);
...
async function canEditQueue(uid, data, action) {
  const [canEditQueue, queuedPost] = await Promise.all([posts.canEditQueue(uid, data, action), posts.getFromQueue(data.id)]);
  if (!queuedPost) {
    throw new Error('[[error:no-post]]');
  }
  if (!canEditQueue) {
...
async function sendQueueNotification(type, targetUid, path, notificationText) {
  const bodyShort = notificationText ? translator.compile(`notifications:${type}`, notificationText) : translator.compile(`notifications:${type}`);
  const notifData = {
    type: type,
