...
exports.setDefaultPostData = function (reqOrSocket, data) {
  ...
  data.req = exports.buildReqObject(reqOrSocket, {
    ...data
  });
...
exports.doTopicAction = async function (action, event, caller, {
  ...
  await Promise.all(tids.map(async tid => {
    const {
      ...
      cid,
      mainPid
    } = await topics.getTopicFields(tid, ['title', 'cid', 'mainPid']);
      ...
      case 'purge':
        {
          if (utils.isNumber(cid) && parseInt(cid, 10) > 0) {
            ...
            activitypub.out.delete.note(caller.uid, mainPid);
            activitypub.out.undo.announce('cid', cid, tid);
          }
        }
    }
  }));
