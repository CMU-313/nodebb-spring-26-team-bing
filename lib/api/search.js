...
searchApi.categories = async (caller, data) => {
  ...
  data.states = (data.states || ['watching', 'tracking', 'notwatching', 'ignoring']).map(state => categories.watchStates[state]);
  data.parentCid = parseInt(data.parentCid || 0, 10);
  if (data.search) {
    ({
      cids,
      matchedCids
    } = await findMatchedCids(caller.uid, data));
  } else {
...
searchApi.roomUsers = async (caller, {
  query,
  roomId
}) => {
...
searchApi.roomMessages = async (caller, {
  query,
  roomId,
  uid
}) => {
  const [roomData, inRoom] = await Promise.all([messaging.getRoomData(roomId), messaging.isUserInRoom(caller.uid, roomId)]);
  if (!roomData) {
  ...
  const {
    ids
  } = await plugins.hooks.fire('filter:messaging.searchMessages', {
    content: query,
    roomId: [roomId],
    uid: [uid],
    matchWords: 'any',
