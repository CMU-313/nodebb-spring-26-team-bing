...
usersAPI.deleteMany = async function (caller, data) {
  ...
  if (await canDeleteUids(data.uids)) {
    await Promise.all(data.uids.map(uid => processDeletion({
      uid,
      method: 'delete',
      caller
    })));
...
usersAPI.getStatus = async (caller, {
  uid
}) => {
...
usersAPI.getPrivateRoomId = async (caller, {
  uid
} = {}) => {
  if (!uid) {
    throw new Error('[[error:invalid-data]]');
...
usersAPI.changePassword = async function (caller, data) {
  await user.changePassword(caller.uid, Object.assign(data, {
    ip: caller.ip
  }));
  await events.log({
...
usersAPI.generateToken = async (caller, {
  uid,
  description
}) => {
  const api = require('.');
  ...
  const tokenObj = await api.utils.tokens.generate({
    uid,
    description
  });
  return tokenObj.token;
};
usersAPI.deleteToken = async (caller, {
  uid,
  token
}) => {
  const api = require('.');
...
usersAPI.revokeSession = async (caller, {
  uid,
  uuid
}) => {
...
usersAPI.getInviteGroups = async (caller, {
  uid
}) => {
  if (parseInt(uid, 10) !== parseInt(caller.uid, 10)) {
    throw new Error('[[error:no-privileges]]');
...
usersAPI.addEmail = async (caller, {
  email,
  skipConfirmation,
  uid
}) => {
...
usersAPI.listEmails = async (caller, {
  uid
}) => {
  const [isPrivileged, {
    showemail
  }] = await Promise.all([user.isPrivileged(caller.uid), user.getSettings(uid)]);
...
usersAPI.getEmail = async (caller, {
  uid,
  email
}) => {
  const [isPrivileged, {
    showemail
  }, exists] = await Promise.all([user.isPrivileged(caller.uid), user.getSettings(uid), db.isSortedSetMember('email:uid', email.toLowerCase())]);
  const isSelf = caller.uid === parseInt(uid, 10);
  return exists && (isSelf || isPrivileged || showemail);
};
usersAPI.confirmEmail = async (caller, {
  uid,
  email,
  sessionId
}) => {
  const [pending, current, canManage] = await Promise.all([user.email.isValidationPending(uid, email), user.getUserField(uid, 'email'), privileges.admin.can('admin:users', caller.uid)]);
  if (!canManage) {
    throw new Error('[[error:no-privileges]]');
  }
  if (pending) {
...
const exportMetadata = new Map([['posts', ['csv', 'text/csv']], ['uploads', ['zip', 'application/zip']], ['profile', ['json', 'application/json']]]);
...
usersAPI.checkExportByType = async (caller, {
  uid,
  type
}) => await prepareExport({
  uid,
...
usersAPI.getExportByType = async (caller, {
  uid,
  type
}) => {
  const [extension, mime] = exportMetadata.get(type);
  const filename = `${uid}_${type}.${extension}`;
  const exists = await prepareExport({
    uid,
    type
  });
  if (exists) {
...
usersAPI.generateExport = async (caller, {
  ...
  child.on('exit', async () => {
    ...
    const {
      displayname
    } = await user.getUserFields(uid, ['username']);
