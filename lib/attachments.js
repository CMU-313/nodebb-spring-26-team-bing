...
Attachments.update = async (pid, attachments) => {
  ...
  const hashes = [];
  attachments.filter(Boolean).forEach(({
    _type,
    mediaType,
    href,
    url,
  ...
  await Promise.all([db.setObjectBulk(bulkOps.hash), db.setObjectField(`post:${pid}`, 'attachments', hashes.join(',')), posts.clearCachedPost(pid)]);
};
Attachments.empty = async pids => {
  ...
  await Promise.all([db.deleteAll(keys.map(hash => `attachment:${hash}`)), db.deleteObjectFields(postKeys, ['attachments'])]);
};