...
module.exports = function (Posts) {
  ...
  Posts.uploads.sync = async function (pid) {
    const [postData, isMainPost] = await Promise.all([Posts.getPostFields(pid, ['content', 'uploads']), Posts.isMain(pid)]);
  ...
  Posts.uploads.associate = async function (pid, filePaths) {
    ...
    await Promise.all([db.setObjectField(`post:${pid}`, 'uploads', JSON.stringify(currentUploads)), db.sortedSetAddBulk(bulkAdd), Posts.uploads.saveSize(filePaths)]);
  };
  Posts.uploads.dissociate = async function (pid, filePaths) {
    ...
    const promises = [db.setObjectField(`post:${pid}`, 'uploads', JSON.stringify(currentUploads)), db.sortedSetRemoveBulk(bulkRemove)];
    await Promise.all(promises);
    if (!meta.config.preserveOrphanedUploads) {
      const deletePaths = (await Promise.all(filePaths.map(async filePath => (await Posts.uploads.isOrphan(filePath)) ? filePath : false))).filter(Boolean);
      const uploaderUids = (await db.getObjectsFields(deletePaths.map(path => `upload:${md5(path)}`, ['uid']))).map(o => o ? o.uid || null : null);
      await Promise.all(uploaderUids.map((uid, idx) => uid && isFinite(uid) ? user.deleteUpload(uid, uid, deletePaths[idx]) : null).filter(Boolean));
