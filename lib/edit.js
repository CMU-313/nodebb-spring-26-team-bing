...
module.exports = function (Posts) {
  ...
  Posts.edit = async function (data) {
    ...
    const topicData = await topics.getTopicFields(postData.tid, ['cid', 'mainPid', 'title', 'timestamp', 'scheduled', 'slug', 'tags', 'thumbs']);
    await scheduledTopicCheck(data, topicData);
    data.content = data.content === null ? postData.content : data.content;
    const oldContent = postData.sourceContent || postData.content;
    ...
    const contentChanged = (data.sourceContent || data.content) !== oldContent || topic.renamed || topic.tagsupdated;
  ...
  async function editMainPost(data, postData, topicData) {
    ...
    if (Object.prototype.hasOwnProperty.call(data, 'visibilityMode') || Object.prototype.hasOwnProperty.call(data, 'anonymous')) {
    ...
    const tagsupdated = Array.isArray(data.tags) && !_.isEqual(data.tags, topicData.tags.map(tag => tag.value));
    if (tagsupdated) {
    ...
    const thumbsupdated = Array.isArray(data.thumbs) && !_.isEqual(data.thumbs, topicData.thumbs);
    if (thumbsupdated) {
