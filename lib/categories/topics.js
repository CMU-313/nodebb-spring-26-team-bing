...
module.exports = function (Categories) {
  ...
  Categories.getTopicIds = async function (data) {
    const [pinnedTids, set] = await Promise.all([Categories.getPinnedTids({
      ...data,
      start: 0,
      stop: -1
  ...
  Categories.onNewPostMade = async function (cid, pinned, postData) {
    ...
    const promises = [db.sortedSetAdd(`cid:${cid}:pids`, postData.timestamp, postData.pid), db.incrObjectField(`${utils.isNumber(cid) ? 'category' : 'categoryRemote'}:${cid}`, 'post_count')];
    if (!pinned) {
  ...
  Categories.onTopicsMoved = async cids => {
    await Promise.all(cids.map(async cid => {
      await Promise.all([Categories.setCategoryField(cid, 'topic_count', await db.sortedSetCard(`cid:${cid}:tids:lastposttime`)), Categories.setCategoryField(cid, 'post_count', await db.sortedSetCard(`cid:${cid}:pids`)), Categories.updateRecentTidForCid(cid)]);
    }));
  };
  ...
  Categories.sortTidsBySet = async (tids, sort) => {
    ...
    const sorted = tids.map((tid, idx) => [tid, scores[idx]]).sort(([, a], [, b]) => b - a).map(([tid]) => tid);
    return sorted;
  };
};