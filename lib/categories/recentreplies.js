...
module.exports = function (Categories) {
  ...
  Categories.updateRecentTid = async function (cid, tid) {
    const [count, numRecentReplies] = await Promise.all([db.sortedSetCard(`cid:${cid}:recent_tids`), db.getObjectField(`${utils.isNumber(cid) ? 'category' : 'categoryRemote'}:${cid}`, 'numRecentReplies')]);
  ...
  function assignTopicsToCategories(categories, topics) {
    categories.forEach(category => {
      if (category) {
        category.posts = topics.filter(t => t.cid && (t.cid === category.cid || t.parentCids && t.parentCids.includes(category.cid))).sort((a, b) => b.timestamp - a.timestamp).slice(0, parseInt(category.numRecentReplies, 10));
      }
    });
    topics.forEach(t => {
  ...
  Categories.moveRecentReplies = async function (tid, oldCid, cid) {
    ...
    await batch.processArray(pids, async pids => {
      ...
      await Promise.all([db.sortedSetRemove(`cid:${oldCid}:pids`, pids), db.sortedSetAdd(`cid:${cid}:pids`, timestamps, postsToReAdd.map(p => p.pid)), db.sortedSetRemoveBulk(bulkRemove), db.sortedSetAddBulk(bulkAdd)]);
    }, {
      batch: 500
    });
  };
