...
Categories.getCategoryById = async function (data) {
  ...
  const promises = [data.cid !== '-1' ? Categories.getCategoryTopics(data) : [], Categories.getTopicCount(data), Categories.getWatchState([data.cid], data.uid), getChildrenTree(category, data.uid), !utils.isNumber(data.cid) ? activitypub.actors.getLocalFollowers(data.cid) : null];
...
Categories.getCategories = async function (cids) {
  ...
  const [categories, tagWhitelist] = await Promise.all([Categories.getCategoriesData(cids), Categories.getTagWhitelist(cids)]);
