...
module.exports = function (Categories) {
  ...
  async function updateParent(cid, newParent) {
    ...
    await Promise.all([db.sortedSetRemove(`cid:${oldParent}:children`, cid), db.sortedSetAdd(`cid:${newParent}:children`, categoryData.order, cid), db.setObjectField(`${utils.isNumber(cid) ? 'category' : 'categoryRemote'}:${cid}`, 'parentCid', newParent)]);
  ...
  async function updateTagWhitelist(cid, tags) {
    tags = tags.split(',').map(tag => utils.cleanUpTag(tag, meta.config.maximumTagLength)).filter(Boolean);
  ...
  async function updateOrder(cid, order) {
    ...
    await db.sortedSetAdd(`cid:${parentCid}:children`, childrenCids.map((cid, index) => index + 1), childrenCids);
    await db.setObjectBulk(childrenCids.map((cid, index) => [`${utils.isNumber(cid) ? 'category' : 'categoryRemote'}:${cid}`, {
      order: index + 1
    }]));
  ...
  async function updateHandle(cid, handle) {
    ...
    await Promise.all([db.setObjectField(`category:${cid}`, 'handle', handle), db.sortedSetAdd('categoryhandle:cid', cid, handle)]);
  }
};