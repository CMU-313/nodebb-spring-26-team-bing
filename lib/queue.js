...
module.exports = function (Posts) {
  Posts.getQueuedPosts = async (filter = {}, options = {}) => {
    ...
    if (!postData) {
      ...
      const userData = await user.getUsersFields(uids, ['username', 'userslug', 'picture', 'joindate', 'postcount', 'reputation']);
    ...
    if (filter.tid) {
      ...
      postData = postData.filter(item => item && item.data.tid && tids.includes(String(item.data.tid)));
    }
    return postData;
  };
  ...
  Posts.canUserPostContentWithLinks = async function (uid, content) {
    ...
    const [reputation, isPrivileged] = await Promise.all([user.getUserField(uid, 'reputation'), user.isPrivileged(uid)]);
  ...
  Posts.shouldQueue = async function (uid, data) {
    ...
    if (shouldQueue) {
      const [userData, isPrivileged, isMemberOfExempt, categoryQueueEnabled] = await Promise.all([user.getUserFields(uid, ['uid', 'reputation', 'postcount']), user.isPrivileged(uid), groups.isMemberOfAny(uid, meta.config.groupsExemptFromPostQueue), isCategoryQueueEnabled(data)]);
      shouldQueue = categoryQueueEnabled && !isPrivileged && !isMemberOfExempt && (!userData.uid || userData.reputation < meta.config.postQueueReputationThreshold || userData.postcount <= 0 || !(await Posts.canUserPostContentWithLinks(uid, data.content)));
    }
  ...
  Posts.removeFromQueue = async function (id) {
    ...
    const result = await plugins.hooks.fire('filter:post-queue:removeFromQueue', {
      data: data
    });
  ...
  Posts.submitFromQueue = async function (id) {
    ...
    const result = await plugins.hooks.fire('filter:post-queue:submitFromQueue', {
      data: data
    });
    data = result.data;
    if (data.type === 'topic') {
    ...
    await removeFromQueue(id);
    plugins.hooks.fire('action:post-queue:submitFromQueue', {
      data: data
    });
    return data;
  };
  ...
  Posts.editQueuedContent = async function (uid, editData) {
    const [canEditQueue, data] = await Promise.all([Posts.canEditQueue(uid, editData, 'edit'), getParsedObject(editData.id)]);
    if (!data) {
  ...
  Posts.updateQueuedPostsTopic = async function (newTid, tids) {
    const postData = await Posts.getQueuedPosts({
      tid: tids
    }, {
      metadata: false
    });
    if (postData.length) {
      ...
      await db.setObjectBulk(postData.map(p => [`post:queue:${p.id}`, {
