...
module.exports = function (Posts) {
  ...
  Diffs.delete = async function (pid, timestamp, uid) {
    ...
    const [post, diffs, timestamps] = await Promise.all([Posts.getPostSummaryByPids([pid], uid, {
      parse: false,
      escape: true
  ...
  async function postDiffLoad(pid, since, uid) {
    const [post, diffs] = await Promise.all([Posts.getPostSummaryByPids([pid], uid, {
      parse: false,
      escape: true
    ...
    if (tagDiffs.length && post[0].topic) {
      const tags = tagDiffs[tagDiffs.length - 1].tags.split(',').map(tag => ({
        value: tag
      }));
