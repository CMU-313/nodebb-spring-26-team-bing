...
let sanitizeConfig = {
  allowedTags: sanitize.defaults.allowedTags.concat(['ins', 'del', 'img', 'button', 'video', 'audio', 'source', 'iframe', 'embed']),
  allowedAttributes: {
  ...
  nonBooleanAttributes: ['accesskey', 'class', 'contenteditable', 'dir', 'draggable', 'dropzone', 'hidden', 'id', 'lang', 'spellcheck', 'style', 'tabindex', 'title', 'translate', 'aria-*', 'data-*']
};
...
module.exports = function (Posts) {
  ...
  Posts.parsePost = async function (postData, type) {
    ...
    ({
      postData
    } = await plugins.hooks.fire('filter:parse.post', {
      postData,
      type
    }));
      ...
      if (current[1]) {
        try {
          ...
          if (absolute !== current[1]) {
            ...
            content = content.slice(0, current.index + offset) + absolute + content.slice(current.index + offset + current[1].length);
          }
        } catch (err) {
  ...
  Posts.configureSanitize = async () => {
    sanitizeConfig.allowedTags.forEach(tag => {
      sanitizeConfig.allowedAttributes[tag] = _.union(sanitizeConfig.allowedAttributes[tag], sanitizeConfig.nonBooleanAttributes);
    });
