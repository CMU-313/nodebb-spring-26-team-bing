...
module.exports = function (Posts) {
  ...
  async function checkVoteLimitation(pid, uid, type) {
    ...
    const [reputation, isPrivileged, targetUid, votedPidsToday] = await Promise.all([user.getUserField(uid, 'reputation'), user.isPrivileged(uid), Posts.getPostField(pid, 'uid'), db.getSortedSetRevRangeByScore(`uid:${uid}:${type}`, 0, -1, '+inf', Date.now() - oneDay)]);
    if (isPrivileged) {
      return;
    }
  ...
  Posts.updatePostVoteCount = async function (postData) {
    ...
    await Promise.all([updateTopicVoteCount(postData), db.sortedSetAdd('posts:votes', postData.votes, postData.pid), Posts.setPostFields(postData.pid, {
  ...
  async function updateTopicVoteCount(postData) {
    ...
    const promises = [topics.setTopicFields(postData.tid, {
      upvotes: postData.upvotes,
      downvotes: postData.downvotes
