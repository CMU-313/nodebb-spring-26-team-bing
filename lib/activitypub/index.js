...
ActivityPub._constants = Object.freeze({
  ...
  acceptableTypes: ['application/activity+json', 'application/ld+json; profile="https://www.w3.org/ns/activitystreams"'],
...
ActivityPub.resolveId = async (uid, id) => {
  try {
    ...
    ({
      id
    } = await ActivityPub.get('uid', uid, id));
...
ActivityPub.getPublicKey = async (type, id) => {
  ...
  try {
    ({
      publicKey
    } = await db.getObject(`${type}:${id}:keys`));
  } catch (e) {
    ({
      publicKey
    } = await ActivityPub.helpers.generateKeys(type, id));
  }
  return publicKey;
};
...
ActivityPub.verify = async req => {
  ...
  try {
    ...
    const {
      publicKeyPem
    } = await ActivityPub.fetchPublicKey(keyId);
...
ActivityPub.send = async (type, id, targets, payload) => {
  ...
  if (process.env.hasOwnProperty('CI')) {
    ActivityPub._sent.set(payload.id, {
      payload,
      targets
    });
    return;
  ...
  batch.processArray(inboxes, async inboxBatch => {
    ...
    if (retryQueueAdd.length) {
      await Promise.all([db.sortedSetAddBulk(retryQueueAdd), db.setObjectBulk(retryQueuedSet)]);
    }
  }, {
    batch: 50,
    interval: 100
...
async function retryFailedMessages() {
  ...
  await Promise.all(queuedData.map(async (data, index) => {
    ...
    if (ok) {
      ...
      const timeout = 2 ** nextAttempt * oneMinute;
  ...
  await Promise.all([db.sortedSetAddBulk(retryQueueAdd), db.setObjectBulk(retryQueuedSet), db.sortedSetRemove('ap:retry:queue', queueIdsToRemove), db.deleteAll(queueIdsToRemove.map(id => `ap:retry:queue:${id}`))]);
}
ActivityPub.record = async ({
  id,
  type,
  actor
}) => {
  ...
  await Promise.all([db.sortedSetAdd(`activities:datetime`, now, id), ActivityPub.instances.log(hostname), analytics.increment(['activities', `activities:byType:${type}`, `activities:byHost:${hostname}`])]);
};
ActivityPub.buildRecipients = async function (object, {
  pid,
  uid,
  cid
}) {
  ...
  if (pid) {
    ...
    const participants = (await db.getSortedSetMembers(`tid:${tid}:posters`)).filter(uid => !utils.isNumber(uid));
    const announcers = (await ActivityPub.notes.announce.list({
      pid
    })).map(({
      actor
    }) => actor);
    const auxiliaries = Array.from(new Set([...participants, ...announcers]));
    const auxiliaryFollowers = (await user.getUsersFields(auxiliaries, ['followersUrl'])).filter(o => o.hasOwnProperty('followersUrl')).map(({
      followersUrl
    }) => followersUrl);
  ...
  try {
    ...
    if (headers && headers.link) {
      ...
      links.forEach(link => {
        ...
        parts = parts.map(p => p.trim()).reduce((memo, cur) => {
          cur = cur.split('=');
          if (cur.length < 2) {
            cur.push('');
          }
...
ActivityPub.probe = async ({
  ...
  const [isNote, isMessage, isActor, isActorUrl] = await Promise.all([posts.exists(url), messaging.messageExists(url), db.isSortedSetMember('usersRemote:lastCrawled', url), db.isObjectField('remoteUrl:uid', url)]);
  switch (true) {
    case isNote:
      {
    ...
    case isMessage:
      {
        if (uid) {
          const {
            roomId
          } = await messaging.getMessageFields(url, ['roomId']);
