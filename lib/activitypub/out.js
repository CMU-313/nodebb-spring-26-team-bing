...
Out.follow = enabledCheck(async (type, id, actor) => {
  ...
  if (actor.includes('@')) {
    const [uid, cid] = await Promise.all([user.getUidByUserslug(actor), categories.getCidByHandle(actor)]);
    actor = uid || cid;
  }
...
Out.create.note = enabledCheck(async (uid, post) => {
  if (utils.isNumber(post)) {
    post = (await posts.getPostSummaryByPids([post], uid, {
      stripTags: false
    })).pop();
    if (!post) {
      return;
  ...
  const {
    activity,
    targets
  } = await activitypub.mocks.activities.create(pid, uid, post);
  await Promise.all([activitypub.send('uid', uid, Array.from(targets), activity), activitypub.feps.announce(pid, activity)]);
});
Out.create.privateNote = enabledCheck(async messageObj => {
  ...
  targets = targets.filter(uid => !utils.isNumber(uid));
  const object = await activitypub.mocks.notes.private({
    messageObj
  });
  const payload = {
...
Out.update.profile = enabledCheck(async (uid, actorUid) => {
  ...
  const [object, targets] = await Promise.all([activitypub.mocks.actors.user(uid), db.getSortedSetMembers(`followersRemote:${uid}`)]);
...
Out.update.category = enabledCheck(async cid => {
  ...
  const [object, targets] = await Promise.all([activitypub.mocks.actors.category(cid), activitypub.notes.getCategoryFollowers(cid)]);
...
Out.update.note = enabledCheck(async (uid, post) => {
  ...
  const {
    ...
    cc,
    targets
  } = await activitypub.buildRecipients(object, {
    pid: post.pid,
    uid: post.user.uid
  });
  object.to = to;
  ...
  await Promise.all([activitypub.send('uid', uid, Array.from(targets), payload), activitypub.feps.announce(post.pid, payload)]);
});
Out.update.privateNote = enabledCheck(async (uid, messageObj) => {
  ...
  uids = uids.filter(uid => String(uid) !== String(messageObj.fromuid));
  const to = uids.map(uid => utils.isNumber(uid) ? `${nconf.get('url')}/uid/${uid}` : uid);
  const targets = uids.filter(uid => !utils.isNumber(uid));
  const object = await activitypub.mocks.notes.private({
    messageObj
  });
  const payload = {
...
Out.delete.note = enabledCheck(async (uid, pid) => {
  ...
  const post = (await posts.getPostSummaryByPids([pid], uid, {
    stripTags: false
  })).pop();
  const object = await activitypub.mocks.notes.public(post);
  const {
  ...
  await Promise.all([activitypub.send('uid', uid, Array.from(targets), payload), activitypub.feps.announce(pid, payload)]);
});
Out.like = {};
Out.like.note = enabledCheck(async (uid, pid) => {
  ...
  await Promise.all([activitypub.send('uid', uid, [recipient], payload), activitypub.feps.announce(pid, payload)]);
});
Out.dislike = {};
...
Out.announce.topic = enabledCheck(async (tid, uid) => {
  const {
    mainPid: pid,
    cid
  } = await topics.getTopicFields(tid, ['mainPid', 'cid']);
  if (uid) {
  ...
  const {
    ...
    cc,
    targets
  } = await activitypub.buildRecipients({
    id: pid,
    to: [activitypub._constants.publicAddress]
  }, uid ? {
    uid
...
Out.flag = enabledCheck(async (uid, flag) => {
  ...
  const reason = flag.reason || flag.reports && flag.reports.filter(report => report.reporter.uid === uid).at(-1).value;
...
Out.undo.follow = enabledCheck(async (type, id, actor) => {
  ...
  const [isFollowing, isPending] = await Promise.all([db.isSortedSetMember(type === 'uid' ? `followingRemote:${id}` : `cid:${id}:following`, actor), db.isSortedSetMember(`followRequests:${type === 'uid' ? 'uid' : 'cid'}.${id}`, actor)]);
  if (!isFollowing && !isPending) {
    return;
  }
  const timestamps = await db.sortedSetsScore([`followRequests:${type}.${id}`, type === 'uid' ? `followingRemote:${id}` : `cid:${id}:following`], actor);
  ...
  if (type === 'uid') {
    await Promise.all([db.sortedSetRemove(`followingRemote:${id}`, actor), db.sortedSetRemove(`followRequests:uid.${id}`, actor), db.sortedSetRemove(`followersRemote:${actor}`, id), db.decrObjectField(`user:${id}`, 'followingRemoteCount')]);
  } else if (type === 'cid') {
    await Promise.all([db.sortedSetRemove(`cid:${id}:following`, actor), db.sortedSetRemove(`followRequests:cid.${id}`, actor), db.sortedSetRemove(`followersRemote:${actor}`, `cid|${id}`)]);
  }
});
Out.undo.like = enabledCheck(async (uid, pid) => {
  ...
  await Promise.all([activitypub.send('uid', uid, [author], payload), activitypub.feps.announce(pid, payload)]);
});
...
Out.undo.announce = enabledCheck(async (type, id, tid) => {
  ...
  const exists = await Promise.all([topics.exists(tid), type === 'uid' ? user.exists(id) : categories.exists(id)]);
  ...
  const {
    uid,
    mainPid: pid
  } = await topics.getTopicFields(tid, ['uid', 'mainPid']);
