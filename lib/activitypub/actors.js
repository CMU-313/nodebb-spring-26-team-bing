...
Actors.qualify = async (ids, options = {}) => {
  ...
  ids = await Promise.all(ids.map(async id => {
    ...
    if (activitypub.helpers.isWebfinger(id)) {
      ...
      ({
        actorUri: id
      } = await activitypub.helpers.query(id));
    }
...
Actors.assert = async (ids, options = {}) => {
  ...
  let actors = await Promise.all(ids.map(async id => {
    try {
      ...
      } = await activitypub.helpers.query(`${actor.preferredUsername}@${domain}`);
      if (id !== canonicalId) {
        return null;
      }
      let typeOk = false;
      ...
      if (!typeOk || !activitypub._constants.requiredActorProps.every(prop => actor.hasOwnProperty(prop))) {
        return null;
      }
      try {
        const [followers, following] = await Promise.all([actor.followers ? activitypub.get('uid', 0, actor.followers) : {
          totalItems: 0
        }, actor.following ? activitypub.get('uid', 0, actor.following) : {
          totalItems: 0
        }]);
  ...
  await Promise.all([db.sortedSetRemoveBulk(queries.searchRemove), db.deleteObjectFields('handle:uid', queries.handleRemove)]);
  await Promise.all([db.setObjectBulk(bulkSet), db.sortedSetAdd('usersRemote:lastCrawled', profiles.map(() => now), profiles.map(p => p.uid)), db.sortedSetAddBulk(queries.searchAdd), db.setObject('handle:uid', queries.handleAdd)]);
  if (categories.size) {
...
Actors.assertGroup = async (ids, options = {}) => {
  ...
  let groups = await Promise.all(ids.map(async id => {
    try {
      ...
      const typeOk = Array.isArray(actor.type) ? actor.type.some(type => activitypub._constants.acceptableGroupTypes.has(type)) : activitypub._constants.acceptableGroupTypes.has(actor.type);
      if (!typeOk || !activitypub._constants.requiredActorProps.every(prop => actor.hasOwnProperty(prop))) {
        return null;
      }
      const url = Array.isArray(actor.url) ? actor.url.shift() : actor.url;
  ...
  await Promise.all([db.sortedSetRemoveBulk(queries.searchRemove), db.deleteObjectFields('handle:cid', queries.handleRemove)]);
  ...
  await Promise.all([db.setObjectBulk(bulkSet), db.sortedSetAdd('usersRemote:lastCrawled', groups.map(() => now), groups.map(p => p.id)), db.sortedSetAddBulk(queries.searchAdd), db.setObject('handle:cid', queries.handleAdd), db.setsAdd(masksAdd, 'topics:create'), db.setsRemove(masksRemove, 'topics:create')]);
  return categoryObjs;
};
...
Actors.getLocalFollowCounts = async actors => {
  ...
  const [followersCounts, followingCounts] = await Promise.all([db.sortedSetsCard(followerKeys), db.sortedSetsCard(followingKeys)]);
  const actorToCounts = _.zipObject(validActors, validActors.map((a, idx) => ({
...
Actors.remove = async id => {
  ...
  await Promise.all([db.sortedSetRemoveBulk(bulkRemove), db.deleteObjectField('handle:uid', username), db.deleteObjectField('followersUrl:uid', followersUrl), db.deleteObjectField('remoteUrl:uid', url), db.delete(`userRemote:${id}:keys`)]);
  await Promise.all([db.delete(`userRemote:${id}`), db.sortedSetRemove('usersRemote:lastCrawled', id)]);
};
Actors.removeGroup = async id => {
  ...
  let {
    ...
    url,
    followersUrl
  } = await categories.getCategoryFields(id, ['slug', 'name', 'url', 'followersUrl']);
  slug = slug.toLowerCase();
  const bulkRemove = [['categories:name', `${slug}:${id}`]];
  if (name) {
    bulkRemove.push(['categories:name', `${name.toLowerCase()}:${id}`]);
  }
  await Promise.all([db.sortedSetRemoveBulk(bulkRemove), db.deleteObjectField('handle:cid', slug), db.deleteObjectField('followersUrl:cid', followersUrl), db.deleteObjectField('remoteUrl:cid', url), db.delete(`categoryRemote:${id}:keys`)]);
  await Promise.all([db.delete(`categoryRemote:${id}`), db.sortedSetRemove('usersRemote:lastCrawled', id)]);
};
Actors.prune = async () => {
  ...
  await batch.processArray(ids, async ids => {
    const exists = await Promise.all([db.exists(ids.map(id => `userRemote:${id}`)), db.exists(ids.map(id => `categoryRemote:${id}`))]);
    let uids = new Set();
    let cids = new Set();
    ...
    const [postCounts, roomCounts, followCounts] = await Promise.all([db.sortedSetsCard(uids.map(uid => `uid:${uid}:posts`)), db.sortedSetsCard(uids.map(uid => `uid:${uid}:chat:rooms`)), Actors.getLocalFollowCounts(uids)]);
    ...
    if (cleanupUids.length) {
      await Promise.all([db.sortedSetRemove('usersRemote:lastCrawled', cleanupUids), db.deleteAll(cleanupUids.map(uid => `userRemote:${uid}`))]);
