    ...
    case Array.isArray(attributedTo):
      {
        ...
        attributedTo = attributedTo.shift();
        break;
      }
...
Mocks.profile = async actors => {
  const profiles = await Promise.all(actors.map(async actor => {
    ...
    if (tag && Array.isArray(tag)) {
      tag.filter(tag => tag.type === 'Emoji' && isEmojiShortcode.test(tag.name) && tag.icon && tag.icon.mediaType && tag.icon.mediaType.startsWith('image/')).forEach(tag => {
        summary = summary.replace(new RegExp(tag.name, 'g'), `<img class="not-responsive emoji" src="${tag.icon.url}" title="${tag.name}" />`);
      });
    }
    const customFields = actor.attachment && Array.isArray(actor.attachment) && actor.attachment.length ? actor.attachment.filter(attachment => activitypub._constants.acceptable.customFields.has(attachment.type)).reduce((map, {
      type,
      name,
      value,
      href,
...
Mocks.post = async objects => {
  ...
  const posts = await Promise.all(objects.map(async object => {
    ...
    if (!activitypub._constants.acceptedPostTypes.includes(object.type) || !activitypub.helpers.isUri(object.id)) {
      return null;
    }
    let {
      id: pid,
...
Mocks.actors.user = async uid => {
  ...
  fields = await Promise.all(fields.map(async field => {
    const [name, value] = await Promise.all([translator.translate(field.name), translator.translate(field.value)]);
    field = {
      ...field,
      ...{
        name,
  ...
  return {
    ...{
      ...
      type: 'Person',
      name: username !== displayname ? fullname : username,
...
Mocks.actors.category = async cid => {
  const [{
    ...
    slug,
    descriptionParsed: summary,
    backgroundImage
  }, publicKey, canPost] = await Promise.all([categories.getCategoryFields(cid, ['name', 'handle', 'slug', 'description', 'descriptionParsed', 'backgroundImage']), activitypub.getPublicKey('cid', cid), privileges.categories.can('topics:create', cid, -2)]);
  let icon;
  if (backgroundImage) {
  ...
  return {
    '@context': ['https://www.w3.org/ns/activitystreams', 'https://w3id.org/security/v1', 'https://join-lemmy.org/context.json'],
...
Mocks.notes.public = async post => {
  ...
  ({
    titleRaw: name
  } = await topics.getTopicFields(post.tid, ['title']));
  if (post.toPid) {
  ...
  let source = null;
  const [markdownEnabled, mentionsEnabled] = await Promise.all([plugins.isActive('nodebb-plugin-markdown'), plugins.isActive('nodebb-plugin-mentions')]);
  if (markdownEnabled) {
    const _post = {
      ...post
    };
    ...
    let {
      content
    } = await posts.parsePost(_post, 'markdown');
  ...
  if (mentionsEnabled) {
    ...
    if (matches.size) {
      tag = tag || [];
      tag.push(...Array.from(matches).map(({
        type,
        id: href,
        slug: name
      }) => {
  ...
  const normalizeAttachment = attachment => attachment.map(({
    mediaType,
    url,
    width,
    height
  ...
  const [uploads, thumbs] = await Promise.all([posts.uploads.listWithSizes(post.pid), topics.getTopicField(post.tid, 'thumbs')]);
  ...
  uploads.forEach(({
    ...
    (noteAttachment || attachment).push({
      mediaType,
      url,
      width,
      height
  ...
  if (isArticle) {
    ...
    const sentences = tokenizer.sentences(post.content, {
  ...
  let audience = utils.isNumber(post.category.cid) ? `${nconf.get('url')}/category/${post.category.cid}` : post.category.cid;
  if (inReplyTo) {
  ...
  ({
    object
  } = await plugins.hooks.fire('filter:activitypub.mocks.note', {
    object,
    post,
    private: false
  }));
...
Mocks.notes.private = async ({
  ...
  let inReplyTo;
  if (messageObj.toMid) {
    inReplyTo = utils.isNumber(messageObj.toMid) ? `${nconf.get('url')}/message/${messageObj.toMid}` : messageObj.toMid;
  }
  if (!inReplyTo) {
...
Mocks.activities.create = async (pid, uid, post) => {
  ...
  if (!post) {
    post = (await posts.getPostSummaryByPids([pid], uid, {
      stripTags: false
    })).pop();
    if (!post) {
      throw new Error('[[error:invalid-pid]]');
