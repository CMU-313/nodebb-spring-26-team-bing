...
Relays.add = async url => {
  ...
  await Promise.all([db.sortedSetAdd('relays:createtime', now, url), db.sortedSetAdd('relays:state', 0, url)]);
};
Relays.remove = async url => {
  ...
  await Promise.all([db.sortedSetRemove('relays:createtime', url), db.sortedSetRemove('relays:state', url)]);
};
