...
Contexts.get = async (uid, id) => {
  ...
  const headers = {};
  if (tid) {
    const [mainPid, pids] = await Promise.all([topics.getTopicField(tid, 'mainPid'), db.getSortedSetMembers(`tid:${tid}:posts`)]);
    pids.push(mainPid);
  ...
  try {
    ...
    ({
      type
    } = await activitypub.get('uid', uid, context));
  } catch (e) {
...
Contexts.getItems = async (uid, id, options) => {
  ...
  if (items) {
    ...
    items = await Promise.all(items.map(async item => activitypub.helpers.isUri(item) ? parseString(uid, item) : parseItem(uid, item)));
  ...
  if (next) {
    ...
    Array.from(await Contexts.getItems(uid, isUrl && next, {
      ...options,
      root: false,
      object: !isUrl && next
    })).forEach(item => {
...
async function parseString(uid, item) {
  const {
    type,
    id
  } = await activitypub.helpers.resolveLocalId(item);
...
async function parseItem(uid, item) {
  const {
    type,
    id
  } = await activitypub.helpers.resolveLocalId(item.id);
