...
Helpers._webfingerCache = webfingerCache;
...
Helpers.query = async id => {
  ...
  try {
    ({
      response,
      body
    } = await request.get(`https://${hostname}/.well-known/webfinger?${query}`, {
      headers: {
...
Helpers.generateKeys = async (type, id) => {
  ...
  await db.setObject(`${type}:${id}:keys`, {
    publicKey,
    privateKey
  });
  return {
...
Helpers.resolveObjects = async ids => {
  ...
  const objects = await Promise.all(ids.map(async id => {
    ...
    switch (type) {
      ...
      case 'post':
        {
          const post = (await posts.getPostSummaryByPids([resolvedId], activitypub._constants.uid, {
            stripTags: false,
            extraFields: ['edited']
          })).pop();
          if (!post) {
      ...
      case 'message':
        {
          ...
          return activitypub.mocks.notes.private({
            messageObj
          });
        }
      default:
...
Helpers.remoteAnchorToLocalProfile = async (content, isMarkdown = false) => {
  ...
  await Promise.all(localUrls.map(async url => {
    const {
      type,
      id
    } = await Helpers.resolveLocalId(url);
    if (type === 'user') {
      urlMap.set(url, id);
    }
  }));
  const [backrefs, urlAsIdExists] = await Promise.all([db.getObjectFields('remoteUrl:uid', urlsArray), db.isSortedSetMembers('usersRemote:lastCrawled', urlsArray)]);
...
Helpers.makeSet = (object, properties) => new Set(properties.reduce((memo, property) => memo.concat(object[property] ? Array.isArray(object[property]) ? object[property] : [object[property]] : []), []));
...
Helpers.generateDigest = set => {
  ...
  return Array.from(set).map(item => sha256(item)).reduce((memo, cur) => {
...
Helpers.addressed = (id, activity) => {
  ...
  const combined = new Set([...(activity.to || []), ...(activity.cc || []), ...(activity.bto || []), ...(activity.bcc || []), ...(activity.audience || [])]);
