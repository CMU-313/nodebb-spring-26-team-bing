...
inbox.create = async req => {
  ...
  const {
    cids
  } = await activitypub.actors.getLocalFollowers(actor);
  let cid = null;
  if (cids.size > 0) {
...
inbox.add = async req => {
  ...
  const {
    type,
    id: pid
  } = await activitypub.helpers.resolveLocalId(object);
  if (type === 'post') {
    ...
    if (context) {
      const {
        attributedTo
      } = await activitypub.get('uid', 0, context);
...
inbox.remove = async req => {
  ...
  const mainPid = await activitypub.contexts.getItems(0, object.id, {
    returnRootId: true
  });
...
inbox.update = async req => {
  ...
  switch (true) {
    case activitypub._constants.acceptedPostTypes.includes(object.type):
      {
        const [isNote, isMessage] = await Promise.all([posts.exists(object.id), messaging.messageExists(object.id)]);
        try {
          switch (true) {
            case isNote:
              {
            ...
            case isMessage:
              {
                const {
                  roomId,
                  deleted
                } = await messaging.getMessageFields(object.id, ['roomId', 'deleted']);
                await messaging.editMessage(actor, object.id, roomId, object.content);
                if (deleted) {
                  await api.chats.restoreMessage({
                    uid: actor
                  }, {
                    mid: object.id
                  });
    ...
    case activitypub._constants.acceptableActorTypes.has(object.type):
      {
        await activitypub.actors.assert(object.id, {
          update: true
        });
        break;
      }
...
inbox.delete = async req => {
  ...
  const [isNote, isContext] = await Promise.all([posts.exists(id), activitypub.contexts.getItems(0, id, {
    returnRootId: true
  })]);
  switch (true) {
    case isNote:
...
inbox.like = async req => {
  ...
  const {
    type,
    id
  } = await activitypub.helpers.resolveLocalId(object.id);
    ...
    case activitypub._constants.acceptedPostTypes.includes(object.type):
      {
        if (String(object.id).startsWith(nconf.get('url'))) {
          ...
          if (!fromRelay && !cid && !syncedCids.length) {
            const {
              followers
            } = await activitypub.actors.getLocalFollowCounts(actor);
            if (!followers) {
          ...
          pid = object.id;
          pid = await activitypub.resolveId(0, pid);
          if (!pid) {
            return;
          }
          const assertion = await activitypub.notes.assert(0, pid, {
            cid,
            skipChecks: true
          });
          if (!assertion) {
          ...
          await activitypub.notes.updateLocalRecipients(pid, {
            to,
            cc
          });
...
inbox.follow = async req => {
  ...
  const {
    type,
    id
  } = await helpers.resolveLocalId(object.id);
  if (type === 'application') {
  ...
  if (type === 'user') {
    const [exists, allowed] = await Promise.all([user.exists(id), privileges.global.can('view:users', activitypub._constants.uid)]);
    if (!exists || !allowed) {
    ...
    await db.sortedSetAdd(`followingRemote:${actor}`, now, id);
    ...
    const [exists, allowed] = await Promise.all([categories.exists(id), privileges.categories.can('read', id, activitypub._constants.uid)]);
    if (!exists) {
...
inbox.accept = async req => {
  ...
  const {
    type: localType,
    id
  } = await helpers.resolveLocalId(object.actor);
  ...
  if (type === 'Follow') {
    if (localType === 'user') {
      if (!(await db.isSortedSetMember(`followRequests:uid.${id}`, actor))) {
        if (await db.isSortedSetMember(`followingRemote:${id}`, actor)) return;
        return reject('Accept', req.body, actor);
      }
      const timestamp = await db.sortedSetScore(`followRequests:uid.${id}`, actor);
      await Promise.all([db.sortedSetRemove(`followRequests:uid.${id}`, actor), db.sortedSetAdd(`followingRemote:${id}`, timestamp, actor), db.sortedSetAdd(`followersRemote:${actor}`, timestamp, id)]);
      ...
      if (!(await db.isSortedSetMember(`followRequests:cid.${id}`, actor))) {
        if (await db.isSortedSetMember(`cid:${id}:following`, actor)) return;
        return reject('Accept', req.body, actor);
      }
      const timestamp = await db.sortedSetScore(`followRequests:cid.${id}`, actor);
      await Promise.all([db.sortedSetRemove(`followRequests:cid.${id}`, actor), db.sortedSetAdd(`cid:${id}:following`, timestamp, actor), db.sortedSetAdd(`followersRemote:${actor}`, timestamp, `cid|${id}`)]);
    }
  }
};
inbox.undo = async req => {
  ...
  let {
    type: localType,
    id
  } = await helpers.resolveLocalId(object.object);
          ...
          case 'user':
            {
              ...
              await Promise.all([db.sortedSetRemove(`followersRemote:${id}`, actor), db.sortedSetRemove(`followingRemote:${actor}`, id)]);
    ...
    case 'Flag':
      {
        ...
        await Promise.all(object.object.map(async subject => {
          const {
            type,
            id
          } = await activitypub.helpers.resolveLocalId(subject.id);
          try {
...
inbox.flag = async req => {
  ...
  await Promise.all(objects.map(async (subject, index) => {
    const {
      type,
      id
    } = await activitypub.helpers.resolveObjects(subject.id);
    try {
...
inbox.reject = async req => {
  ...
  await Promise.all([db.sortedSetRemove('ap:retry:queue', queueId), db.delete(`ap:retry:queue:${queueId}`)]);
};