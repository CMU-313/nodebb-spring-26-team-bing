...
describe('minifier', () => {
	...
	const styles = ['@use "./1" as *;', '@use "./2.scss" as *;'].join('\n');
	...
	*/
	...
	it('should build templates', function (done) {
		...
		build.build(['templates'], (err) => {
			...
			const filename = path.join(
				__dirname,
				'../build/public/templates/admin/header.tpl',
			);
			assert(file.existsSync(filename));
			assert(
				fs.readFileSync(filename).toString().startsWith('<!DOCTYPE html>'),
			);
			done();
		});
	});
	...
	it('should build languages', (done) => {
		build.build(['languages'], (err) => {
			...
			const globalFile = path.join(
				__dirname,
				'../build/public/language/en-GB/global.json',
			);
			assert(file.existsSync(globalFile), 'global.json exists');
			const global = fs.readFileSync(globalFile).toString();
			assert.strictEqual(
				JSON.parse(global).home,
				'Home',
				'global.json contains correct translations',
			);

			const mdFile = path.join(
				__dirname,
				'../build/public/language/en-GB/markdown.json',
			);
			...
			assert.strictEqual(
				JSON.parse(md).bold,
				'bolded text',
				'markdown.json contains correct translations',
			);
