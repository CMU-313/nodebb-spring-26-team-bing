...
describe("Topic's", () => {
	...
	describe("controller", () => {
		...
		it("should redirect if post index is out of range", async () => {
			...
			assert.equal(
				...
				encodeURIComponent(`/topic/${topicData.tid}/topic-for-controller-test`),
			);
	...
	describe("unread", () => {
		...
		it("should not return topic as unread if topic is deleted", async () => {
			...
			assert(
				...
				JSON.stringify({ unreadTids, tid: result.topicData.tid }),
			);
		});
	});
				...
				(err, data) => {
					...
					const tagData = [
						...
						},
						{
							value: "node & c++",
							valueEscaped: "node &amp; c++",
							valueEncoded: "node%20%26%20c%2B%2B",
							score: 1,
							class: "node-&amp;-c++",
						},
						{
	...
	describe("scheduled topics", () => {
		...
		it("should properly update timestamp in cid:<cid>:pids after editing and posting immediately", async () => {
			...
			assert(
				score < Date.now(),
				"Post in cid:<cid>:pids has wrong score, it should not be in the future",
			);
		});
	});
});
