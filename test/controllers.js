...
describe('Controllers', () => {
	...
	describe('routes that should 200/404 etc.', () => {
		...
		const testRoutes = [
			...
			{
				it: 'should load /outgoing?url=<url>',
				url: '/outgoing?url=http://youtube.com',
			},
			{
				it: 'should 404 on /outgoing with no url',
				url: '/outgoing',
				status: 404,
			},
			{
				it: 'should 404 on /outgoing with javascript: protocol',
				url: '/outgoing?url=javascript:alert(1);',
				status: 404,
			},
			{
				it: 'should 404 on /outgoing with invalid url',
				url: '/outgoing?url=derp',
				status: 404,
			},
			...
			{
				it: 'should load category recent posts rss feed',
				url: `/category/1/recentposts.rss`,
			},
			...
			{
				it: 'should load sitemap/categories.xml',
				url: `/sitemap/categories.xml`,
			},
			{ it: 'should load sitemap/topics.1.xml', url: `/sitemap/topics.1.xml` },
			{
				it: 'should load theme screenshot',
				url: `/css/previews/nodebb-theme-harmony`,
			},
		...
		testRoutes.forEach((route) => {
			it(route.it, async () => {
				...
				if (route.body) {
					...
					assert(
						body,
						`No body returned for ${route.url} ${response.statusCode}`,
					);
				}
			});
		});
	...
	it('should load /register/complete', async () => {
		...
		const { response: res2, body: body2 } = await request.get(
	...
	describe('registration interstitials', () => {
		describe('email update', () => {
			...
			it('email interstitial should still apply if empty email entered and requireEmailAddress is enabled', async () => {
				const { response: res } = await request.post(
					`${nconf.get('url')}/register/complete`,
					{
						jar,
						maxRedirect: 0,
						redirect: 'manual',
			...
			it('should throw error if email is not valid', async () => {
				...
				assert.strictEqual(
					result.interstitials[0].template,
					'partials/email_update',
				);
				await assert.rejects(
					result.interstitials[0].callback(
						{ uid },
						{
							email: 'invalidEmail',
						},
			...
			it('should successfully issue validation request if the correct password is passed in', async () => {
				...
				const pending = await user.email.isValidationPending(
					uid,
					`${username}@nodebb.com`,
				);
			...
			describe('blocking access for unconfirmed emails', () => {
				...
				it('should redirect back to interstitial for categories requiring validated email', async () => {
					...
					await privileges.categories.rescind(['groups:read'], cid, [
						'registered-users',
					]);
					await privileges.categories.give(['groups:read'], cid, [
						'verified-users',
					]);
					const { response } = await request.get(
						`${nconf.get('url')}/category/${cid}/${slugify(name)}`,
						{
							jar,
							maxRedirect: 0,
							redirect: 'manual',
				...
				it('should redirect back to interstitial for topics requiring validated email', async () => {
					...
					const { response: res2 } = await request.get(
						`${nconf.get('url')}/topic/${topicData.tid}/${slugify(title)}`,
						{
							jar,
							maxRedirect: 0,
							redirect: 'manual',
		...
		describe('abort behaviour', () => {
			...
			it('should terminate the session and send user back to index if interstitials remain', async () => {
				...
				assert.strictEqual(
					...
					`express.sid=; Path=${nconf.get('relative_path') || '/'}; Expires=Thu, 01 Jan 1970 00:00:00 GMT${nconf.get('secure') ? '; Secure' : ''}; SameSite=Lax`,
				);
				assert.strictEqual(
					response.headers.location,
					`${nconf.get('relative_path')}/`,
	...
	it('should 404 if brand:touchIcon is not valid', async () => {
		...
		const { response, body } = await request.get(
			`${nconf.get('url')}/apple-touch-icon`,
		);
	...
	it('should error if guests do not have search privilege', async () => {
		const { response, body } = await request.get(
			`${nconf.get('url')}/api/users?query=bar&section=sort-posts`,
		);
	...
	it('should load users search page', async () => {
		...
		const { response, body } = await request.get(
			`${nconf.get('url')}/users?query=bar&section=sort-posts`,
		);
		assert.equal(response.statusCode, 200);
		assert(body);
	...
	it('should load group members page', async () => {
		const { response, body } = await request.get(
			`${nconf.get('url')}/groups/group-details/members`,
		);
		assert.equal(response.statusCode, 200);
		assert(body);
	});
	...
	it('should 404 when trying to load group members of hidden group', async () => {
		...
		const { response } = await request.get(
			`${nconf.get('url')}/groups/hidden-group/members`,
		);
		assert.equal(response.statusCode, 404);
	});
	...
	describe('revoke session', () => {
		...
		it("should fail if user doesn't exist", async () => {
			const { response, body } = await request.del(
	...
	describe('account pages', () => {
		...
		it('should redirect to userslug and keep query params', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/uid/${fooUid}/topics?foo=bar`,
			);
			assert.equal(response.statusCode, 200);
			assert.equal(
				response.headers['x-redirect'],
				encodeURIComponent('/user/foo/topics?foo=bar'),
			);
		...
		describe('/me/*', () => {
			it('should redirect to user profile', async () => {
				const { response, body } = await request.get(`${nconf.get('url')}/me`, {
					jar,
				});
				assert.equal(response.statusCode, 200);
				assert(
			...
			it('api should redirect to /user/[userslug]/bookmarks', async () => {
				const { response, body } = await request.get(
					`${nconf.get('url')}/api/me/bookmarks`,
					{ jar },
				);
				assert.equal(response.statusCode, 200);
				assert.equal(
					response.headers['x-redirect'],
					encodeURIComponent('/user/foo/bookmarks'),
				);
			...
			it('api should redirect to /user/[userslug]/edit/username', async () => {
				...
				assert.equal(
					response.headers['x-redirect'],
					encodeURIComponent('/user/foo/edit/username'),
				);
		...
		it('should load /user/foo/posts', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/posts`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/bookmarks', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/bookmarks`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/upvoted', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/upvoted`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/downvoted', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/downvoted`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/best', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/best`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/controversial', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/controversial`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/watched', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/watched`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/ignored', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/ignored`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/topics', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/topics`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/blocks', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/blocks`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/consent', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/consent`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/sessions', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/sessions`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/categories', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/categories`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/tags', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/tags`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		it('should load /user/foo/uploads', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/uploads`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		...
		describe('user data export routes', () => {
			...
			it('should export users posts', async () => {
				const { response, body } = await request.get(
					`${nconf.get('url')}/api/v3/users/${fooUid}/exports/posts`,
					{ jar: jar },
				);
				assert.equal(response.statusCode, 200);
				assert(body);
			...
			it('should export users uploads', async () => {
				const { response, body } = await request.get(
					`${nconf.get('url')}/api/v3/users/${fooUid}/exports/uploads`,
					{ jar: jar },
				);
				assert.equal(response.statusCode, 200);
				assert(body);
			...
			it('should export users profile', async () => {
				const { response, body } = await request.get(
					`${nconf.get('url')}/api/v3/users/${fooUid}/exports/profile`,
					{ jar: jar },
				);
				assert.equal(response.statusCode, 200);
				assert(body);
		...
		it('should load notifications page', async () => {
			...
			await sleep(2500);
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/notifications`,
				{
					jar,
				},
			);
			...
			assert.equal(
				notif.bodyShort,
				'<strong>test1</strong> has posted a reply to: <strong>test2</strong>',
			);
		...
		it('should 404 if user does not exist', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/email/doesnotexist`,
			);
			assert.equal(response.statusCode, 404);
			assert(body);
		});
		...
		it('should load user by uid', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/uid/${fooUid}`,
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		});
		...
		it('should load user by username', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/username/foo`,
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		});
		...
		it('should return false if user can not edit user', async () => {
			...
			let { response } = await request.get(
				`${nconf.get('url')}/api/user/foo/info`,
				{ jar },
			);
			assert.equal(response.statusCode, 403);
			({ response } = await request.get(
				`${nconf.get('url')}/api/user/foo/edit`,
				{ jar },
			));
			assert.equal(response.statusCode, 403);
		});
		...
		it('should 404 if user does not exist', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/doesnotexist`,
				{ jar },
			);
			assert.equal(response.statusCode, 404);
		});
		...
		it('should only return posts that are not deleted', async () => {
			...
			const { pid: pidToDelete } = await topics.reply({
				uid: fooUid,
				content: '1st reply',
				tid: topicData.tid,
			});
		...
		it('should return selected group title', async () => {
			...
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/groupie`,
			);
			assert.equal(response.statusCode, 200);
		...
		it('should render edit/email', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/foo/edit/email`,
				{ jar },
			);
	...
	describe('account follow page', () => {
		...
		before(async () => {
			...
			const isFollowing = await socketUser.isFollowing(
				{ uid: uid },
				{ uid: fooUid },
			);
			assert(isFollowing);
		...
		it('should get following page', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/user/follower/following`,
			);
			assert.equal(response.statusCode, 200);
	...
	describe('post redirect', () => {
		...
		it('should return correct post path', async () => {
			...
			assert.equal(
				response.headers['x-redirect'],
				encodeURIComponent('/topic/1/test-topic-title'),
			);
	...
	describe('cookie consent', () => {
		it('should return relevant data in configs API route', async () => {
			...
			assert.equal(
				translator.escape('[[global:cookies.message]]'),
				body.cookies.message,
			);
			assert.equal(
				translator.escape('[[global:cookies.accept]]'),
				body.cookies.dismiss,
			);
			assert.equal(
				translator.escape('[[global:cookies.learn-more]]'),
				body.cookies.link,
			);
		});
	...
	describe('handle errors', () => {
		...
		it('should handle category malformed uri', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/category/1/a%AFc`,
			);
			assert.equal(response.statusCode, 200);
			assert(body);
		});
		...
		it('should handle api page redirect through error', async () => {
			...
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/users`,
			);
			assert.equal(response.statusCode, 200);
			assert.equal(
				response.headers['x-redirect'],
				encodeURIComponent('/api/popular'),
			);
	...
	describe('category', () => {
		...
		it('should return 404 if cid is not a number', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/category/fail`,
			);
			assert.equal(response.statusCode, 404);
		});
		...
		it('should return 404 if topic index is not a number', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/category/${category.slug}/invalidtopicindex`,
			);
			assert.equal(response.statusCode, 404);
		});
		...
		it('should 404 if category does not exist', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/category/123123`,
			);
			assert.equal(response.statusCode, 404);
		});
		...
		it('should redirect if topic index is negative', async () => {
			const { response } = await request.get(
				`${nconf.get('url')}/api/category/${category.slug}/-10`,
			);
			assert.equal(response.statusCode, 200);
		...
		it('should redirect if category is a link', async () => {
			...
			assert.equal(
				result.response.headers['x-redirect'],
				encodeURIComponent('https://nodebb.org'),
			);
		...
		it('should get recent topic replies from children categories', async () => {
			...
			const childCategory1 = await categories.create({
				name: 'child category 1',
				parentCid: category.cid,
			});
			const childCategory2 = await categories.create({
		...
		it('should load categories', async () => {
			...
			const data = await helpers.getCategories(
				'cid:0:children',
				1,
				'topics:read',
				0,
	...
	describe('unread', () => {
		...
		it('should load unread page', async () => {
			const { response } = await request.get(`${nconf.get('url')}/api/unread`, {
				jar,
			});
			assert.equal(response.statusCode, 200);
		});
		...
		it('should 404 if filter is invalid', async () => {
			const { response } = await request.get(
				`${nconf.get('url')}/api/unread/doesnotexist`,
				{ jar },
			);
			assert.equal(response.statusCode, 404);
		});
		...
		it('should return total unread count', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/unread/total?filter=new`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert.equal(body, 0);
		...
		it('should redirect if page is out of bounds', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/unread?page=-1`,
				{ jar },
			);
			assert.equal(response.statusCode, 200);
			assert.equal(
				response.headers['x-redirect'],
				encodeURIComponent('/unread?page=1'),
			);
	...
	describe('admin middlewares', () => {
		it('should redirect to login', async () => {
			const { response } = await request.get(
				`${nconf.get('url')}/api/admin/advanced/database`,
			);
			assert.equal(response.statusCode, 401);
		});
		...
		it('should redirect to login', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/admin/advanced/database`,
			);
			assert.equal(response.statusCode, 200);
	...
	describe('composer', () => {
		...
		it('should load the composer route', async () => {
			const { response, body } = await request.get(
				`${nconf.get('url')}/api/compose?cid=${cid}`,
				{
					jar,
				},
			);
		...
		it('should create a new topic and reply by composer route as a guest', async () => {
			...
			await privileges.categories.rescind(
				['groups:topics:post', 'groups:topics:reply'],
				cid,
				'guests',
			);
		});
		...
		it('should not load a topic data that is in private category', async () => {
			...
			let { response, body } = await request.get(
				`${nconf.get('url')}/api/compose?tid=${result.topicData.tid}`,
			);
			assert.equal(response.statusCode, 401);
			assert(!body.title);
			...
			({ response, body } = await request.get(
				`${nconf.get('url')}/api/compose?pid=${result.postData.pid}`,
			));
			assert.equal(response.statusCode, 401);
			assert(!body.title);
	...
	describe('test routes', () => {
		if (process.env.NODE_ENV === 'development') {
			...
			it('should load redoc write route', async () => {
				const { response, body } = await request.get(
					`${nconf.get('url')}/debug/spec/write`,
				);
				assert.equal(response.statusCode, 200);
				assert(body);
			});
			...
			it('should load 404 for invalid type', async () => {
				const { response, body } = await request.get(
					`${nconf.get('url')}/debug/spec/doesnotexist`,
				);
				assert.equal(response.statusCode, 404);
				assert(body);
			});
	...
	describe('.well-known', () => {
		describe('webfinger', () => {
			...
			it('should error if resource parameter is missing', async () => {
				const { response } = await request.get(
					`${nconf.get('url')}/.well-known/webfinger`,
				);
			...
			it('should error if resource parameter is malformed', async () => {
				const { response } = await request.get(
					`${nconf.get('url')}/.well-known/webfinger?resource=foobar`,
				);
				assert.strictEqual(response.statusCode, 400);
			});
			...
			it('should deny access if view:users privilege is not enabled for guests', async () => {
				...
				const { response } = await request.get(
					`${nconf.get('url')}/.well-known/webfinger?resource=acct:${username}@${nconf.get('url_parsed').host}`,
				);
				assert.strictEqual(response.statusCode, 404);
			...
			it('should respond appropriately if the user requested does not exist locally', async () => {
				const { response } = await request.get(
					`${nconf.get('url')}/.well-known/webfinger?resource=acct:foobar@${nconf.get('url_parsed').host}`,
				);
				assert.strictEqual(response.statusCode, 404);
			});
			...
			it('should return a valid webfinger response if the user exists', async () => {
				...
				assert(
					body.subject,
					`acct:${username}@${nconf.get('url_parsed').host}`,
				);
			});
		});
	});
