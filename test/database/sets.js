...
describe("Set methods", () => {
	...
	describe("setsAdd()", () => {
		...
		it("should add the values to each set", async () => {
			await db.setsAdd(
				["saddarray1", "saddarray2", "saddarray3"],
				["v1", "v2", "v3"],
			);
			const data = await db.getSetsMembers([
				"saddarray1",
				"saddarray2",
				"saddarray3",
			]);
	...
	describe("isMemberOfSets()", () => {
		...
		it("should return an array of booleans", (done) => {
			db.isMemberOfSets(
				["set1", "testSet1", "set2", "doesnotexist"],
				"value",
				function (err, members) {
					assert.equal(err, null);
					assert.equal(arguments.length, 2);
	...
	describe("setRemove()", () => {
		...
		it("should remove multiple values from multiple keys", async () => {
			...
			const members = await db.getSetsMembers([
				"multiSetTest1",
				"multiSetTest2",
			]);
