...
describe('API', async () => {
	...
	function compare(schema, response, method, path, context) {
		...
		required.forEach((prop) => {
			if (schema.hasOwnProperty(prop)) {
				assert(
					...
					`"${prop}" is a required property (path: ${method} ${path}, context: ${context}), ${JSON.stringify(response)}`,
				);
				...
				switch (schema[prop].type) {
					...
					case 'object': {
						...
						if (
							...
							valid = schema[prop].additionalProperties.oneOf.map(
								({ type }) => type,
							);
						}
						assert(
							valid.includes(typeof response[prop]),
		...
		Object.keys(response).forEach((prop) => {
			...
			assert(
				schema[prop],
				`"${prop}" was found in response, but is not defined in schema (path: ${method} ${path}, context: ${context}), SCHEMA ${JSON.stringify(response)}`,
			);
		});
	}
});
