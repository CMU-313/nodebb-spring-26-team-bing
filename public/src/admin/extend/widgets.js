...
define("admin/extend/widgets", [
	...
	Widgets.init = function () {
		$("#widgets .dropdown .dropdown-menu a").on("click", function (ev) {
			...
			$('#widgets .tab-pane[data-template="' + templateName + '"]').addClass(
				"active",
			);
		...
		$("#widget-selector").on("change", function () {
			...
			$('.available-widgets [data-widget="' + $(this).val() + '"]').removeClass(
				"hide",
			);
		});
			...
			el.addClass("block")
				...
				.droppable({
					...
					drop: function (event, ui) {
						...
						el.find(".card-body .container-html").val(
		...
		$.get(config.relative_path + "/api/admin/extend/widgets", function (data) {
			...
			for (let i = 0; i < areas.length; i += 1) {
				...
				for (let k = 0; k < area.data.length; k += 1) {
					...
					const widgetEl = $(
						'.available-widgets [data-widget="' + widgetData.widget + '"]',
					)
						.clone(true)
						.removeClass("hide");
