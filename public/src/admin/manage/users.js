...
define("admin/manage/users", [
	...
	Users.init = function () {
		...
		$(".export-csv").on("click", function () {
				...
				alerts.alert({
					...
					clickfn: function () {
						window.location.href =
							config.relative_path + "/api/admin/users/csv";
					},
					timeout: 0,
				});
			});
			...
			const defaultFields = [
				{
					label: "[[admin/manage/users:export-field-email]]",
					field: "email",
					selected: true,
				},
				{
					label: "[[admin/manage/users:export-field-username]]",
					field: "username",
					selected: true,
				},
				{
					label: "[[admin/manage/users:export-field-uid]]",
					field: "uid",
					selected: true,
				},
				{
					label: "[[admin/manage/users:export-field-ip]]",
					field: "ip",
					selected: true,
				},
				{
					label: "[[admin/manage/users:export-field-joindate]]",
					field: "joindate",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-lastonline]]",
					field: "lastonline",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-lastposttime]]",
					field: "lastposttime",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-reputation]]",
					field: "reputation",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-postcount]]",
					field: "postcount",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-topiccount]]",
					field: "topiccount",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-profileviews]]",
					field: "profileviews",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-followercount]]",
					field: "followerCount",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-followingcount]]",
					field: "followingCount",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-fullname]]",
					field: "fullname",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-birthday]]",
					field: "birthday",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-signature]]",
					field: "signature",
					selected: false,
				},
				{
					label: "[[admin/manage/users:export-field-aboutme]]",
					field: "aboutme",
					selected: false,
				},
			].concat(
				...
				buttons: {
					submit: {
						...
						callback: function () {
							const fields = modal
								.find("[data-field]")
								.filter((index, el) => $(el).is(":checked"))
								.map((index, el) => $(el).attr("data-field"))
								.get();
							socket.emit(
								"admin.user.exportUsersCSV",
								{ fields },
								function (err) {
									if (err) {
									...
									alerts.alert({
										...
										timeout: Math.max(
											5000,
											(ajaxify.data.userCount / 5000) * 500,
										),
									});
								},
							);
		...
		$('[component="user/select/all"]').on("click", function () {
			$('.users-table [component="user/select/single"]').prop(
				"checked",
				$(this).is(":checked"),
			);
		});
		...
		$(".manage-groups").on("click", function () {
			...
			socket.emit("admin.user.loadGroups", uids, function (err, data) {
				...
				Benchpress.render("admin/partials/manage_user_groups", data).then(
					function (html) {
						const modal = bootbox.dialog({
							message: html,
						...
						modal.on("shown.bs.modal", function () {
							autocomplete.group(
								modal.find(".group-search"),
								function (ev, ui) {
									const uid = $(ev.target).attr("data-uid");
									api
										.put(
											"/groups/" + ui.item.group.slug + "/membership/" + uid,
											undefined,
										)
										.then(() => {
											ui.item.group.nameEscaped = translator.escape(
												ui.item.group.displayName,
											);
						...
						modal.on("click", ".remove-group-icon", function () {
							...
							api
								.del("/groups/" + slugify(groupName) + "/membership/" + uid)
								.then(() => {
									groupCard.remove();
								})
								.catch(alerts.error);
		...
		$(".set-reputation").on("click", function () {
			...
			modal.on("shown.bs.modal", () => {
				modal
					.find("#new-reputation")
					.selectRange(0, modal.find("#new-reputation").val().length);
			});
		});
				...
				function (confirm) {
					if (confirm) {
						Promise.all(
							...
							.then(() => {
								onSuccess(
									"[[admin/manage/users:alerts.ban-success]]",
									".ban",
									true,
								);
							})
			...
			Benchpress.render("modals/temporary-ban", {}).then(function (html) {
				const modal = bootbox.dialog({
					...
					buttons: {
						...
						submit: {
							label:
								"[[admin/manage/users:alerts.button-ban-x, " +
								uids.length +
								"]]",
							callback: function () {
						...
						submit: {
							...
							callback: function () {
								...
								Promise.all(
									...
									.then(() => {
										onSuccess(
											"[[admin/manage/users:alerts.unban-success]]",
											".ban",
											false,
										);
									})
		...
		$(".reset-lockout").on("click", function () {
			...
			socket.emit(
				"admin.user.resetLockouts",
				uids,
				done("[[admin/manage/users:alerts.lockout-reset-success]]"),
			);
		});
		...
		$(".change-email").on("click", function () {
			...
			if (uids.length !== 1) {
				return alerts.error(
					"[[admin/manage/users:alerts.select-a-single-user-to-change-email]]",
				);
			}
			changeEmail.init({
				uid: uids[0],
		...
		$(".validate-email").on("click", function () {
			...
			bootbox.confirm(
				"[[admin/manage/users:alerts.confirm-validate-email]]",
				function (confirm) {
					if (!confirm) {
						return;
					}
					socket.emit("admin.user.validateEmail", uids, function (err) {
						...
						alerts.success(
							"[[admin/manage/users:alerts.validate-email-success]]",
						);
						update(".notvalidated", false);
						update(".pending", false);
						update(".expired", false);
		...
		$(".password-reset-email").on("click", function () {
			...
			bootbox.confirm(
				"[[admin/manage/users:alerts.password-reset-confirm]]",
				function (confirm) {
					if (confirm) {
						socket.emit(
		...
		$(".force-password-reset").on("click", function () {
			...
			bootbox.confirm(
				"[[admin/manage/users:alerts.confirm-force-password-reset]]",
				function (confirm) {
					if (confirm) {
						socket.emit(
							...
							uids,
								...
								"[[admin/manage/users:alerts.validate-force-password-reset-success]]",
							),
						);
					}
				},
		...
		$(".delete-user-content").on("click", () => {
			handleDelete(
				"[[admin/manage/users:alerts.confirm-delete-content]]",
				"/content",
			);
		});
					...
					Promise.all(
						...
						.then(() => {
							if (path !== "/content") {
								...
								alerts.success(
									"[[admin/manage/users:alerts.delete-content-success]]",
								);
							}
							unselectAll();
		...
		function handleUserCreate() {
			$('[data-action="create"]').on("click", function () {
				Benchpress.render("admin/partials/create_user_modal", {}).then(
					function (html) {
						const modal = bootbox.dialog({
							message: html,
	...
	function loadSearchPage(query) {
		...
		$.get(
			config.relative_path + "/api/admin/manage/users?" + qs,
			function (data) {
	...
	function handleFilter() {
		...
		$("#filter-by").on("click", "li", function () {
			...
			$this
				.find("i")
				.toggleClass("fa-check", !$this.find("i").hasClass("fa-check"));
			return false;
		});
