...
define('admin/manage/categories', [
	...
	Categories.init = function () {
		categorySelector.init($('[component="category-selector"]'), {
			parentCid: ajaxify.data.selectedCategory
					...
					save: {
						...
						callback: function () {
							...
							if (val && cid) {
								...
								api
									.put('/categories/' + encodeURIComponent(cid), modified[cid])
									.then(function () {
										ajaxify.refresh();
									})
									.catch(alerts.error);
		...
		function toggleAll(expand) {
			...
			el.find('i')
				.toggleClass('fa-chevron-down', expand)
				.toggleClass('fa-chevron-right', !expand);
			el.closest('[data-cid]')
				.find('> ul[data-cid]')
	...
	Categories.throwCreateModal = function () {
		Benchpress.render('admin/partials/categories/create', {}).then(
			function (html) {
				const modal = bootbox.dialog({
	...
	Categories.throwAddModal = function () {
		Benchpress.render('admin/partials/categories/add', {}).then(
			function (html) {
				const modal = bootbox.dialog({
				...
				function submit() {
					const formData = modal.find('form').serializeObject();
					api
						.post('/api/admin/manage/categories', formData)
						.then(() => {
							ajaxify.refresh();
							modal.modal('hide');
						})
	...
	Categories.remove = function () {
		bootbox.confirm(
			'[[admin/manage/categories:alert.confirm-remove]]',
			(ok) => {
				if (ok) {
					const cid = this.getAttribute('data-cid');
					api
						.del(`/api/admin/manage/categories/${encodeURIComponent(cid)}`)
	...
	Categories.rename = function () {
		bootbox.prompt({
			...
			message:
				'<p class="mb-3">[[admin/manage/categories:alert.rename-help]]</p>',
			callback: (name) => {
				const cid = this.getAttribute('data-cid');
				api
					.post(
						`/api/admin/manage/categories/${encodeURIComponent(cid)}/name`,
						{ name },
					)
					.then(ajaxify.refresh);
			},
	...
	Categories.toggle = function (cids, disabled) {
		...
		Promise.all(
			cids.map((cid) =>
				api
					.put('/categories/' + encodeURIComponent(cid), {
	...
	function itemDragDidEnd(e) {
		...
		if (
			...
			const baseIndex =
				...
				ajaxify.data.categoriesPerPage;
			modified[cid] = {
		...
		function continueRender() {
			app.parseAndTranslate(
				...
				function (html) {
					...
					for (
						let x = 0, numCategories = categories.length;
						x < numCategories;
						x += 1
					) {
						renderList(
