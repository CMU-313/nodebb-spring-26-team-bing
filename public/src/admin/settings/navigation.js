...
define("admin/settings/navigation", [
	...
	navigation.init = function () {
		...
		$("#enabled").on("click", '[name="dropdown"]', function () {
			...
			$(
				'#active-navigation [data-index="' + index + '"] i.dropdown-icon',
	...
	function drop(ev, ui) {
		...
		data.index =
			(parseInt($("#enabled").children().last().attr("data-index"), 10) || 0) +
			1;
		...
		Promise.all([renderNav, renderForm]).then(() => selectIndex(data.index));
	}
	...
	function toggle() {
		...
		btn
			...
			.find('[name="enabled"]')
			.val(disabled ? "on" : "");
		$('#active-navigation [data-index="' + index + '"] a').toggleClass(
			"text-muted",
			!disabled,
		);
		return false;
