...
define("forum/topic/crosspost", [
	...
	function onCategoriesSelected(data) {
		selectedCids = data.selectedCids.filter(
	...
	function crosspost(data) {
		...
		const current = [
			Crosspost.cid,
			...Crosspost.current.map((x) => parseInt(x.cid, 10)),
		];
		...
		const queries = [
			...add.map((cid) => {
				return api.post(`/topics/${data.tid}/crossposts`, { cid });
			}),
			...remove.map((cid) => {
				return api.del(`/topics/${data.tid}/crossposts`, { cid });
			}),
		];
