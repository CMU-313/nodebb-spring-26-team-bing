...
define("forum/groups/details", [
	...
	Details.init = function () {
		...
		detailsPage.on("click", "[data-action]", function () {
			...
			switch (action) {
				...
				case "kick":
					bootbox.confirm(
						...
						function (confirm) {
							...
							api
								.del(
									`/groups/${ajaxify.data.group.slug}/membership/${uid}`,
									undefined,
								)
								.then(() => {
									userRow.remove();
									$('[component="group/member/count"]').text(
										helpers.humanReadableNumber(
											ajaxify.data.group.memberCount - 1,
										),
									);
								})
								.catch(alerts.error);
				...
				case "accept":
					api
						.put(`/groups/${ajaxify.data.group.slug}/pending/${uid}`)
						.then(() => {
							userRow.remove();
							memberList.refresh();
				...
				case "reject":
					api
						.del(`/groups/${ajaxify.data.group.slug}/pending/${uid}`)
						.then(() => {
							userRow.remove();
							memberList.refresh();
							updatePendingAlertVisibility();
				...
				case "acceptInvite":
					api
						.put(`/groups/${ajaxify.data.group.slug}/invites/${app.user.uid}`)
						.then(() => {
							if (uid) {
								userRow.remove();
								memberList.refresh();
				...
				case "rejectInvite":
					api
						.del(
							`/groups/${ajaxify.data.group.slug}/invites/${uid || app.user.uid}`,
						)
						.then(() => {
							if (uid) {
								userRow.remove();
	...
	function handleMemberInvitations() {
		...
		require(["autocomplete"], function (autocomplete) {
			autocomplete.user(searchInput, function (event, selected) {
				api
					.post(
						`/groups/${ajaxify.data.group.slug}/invites/${selected.item.user.uid}`,
					)
					.then(() => updateList())
					.catch(alerts.error);
			});
	...
	function removeCover() {
		bootbox.confirm(
			"[[groups:remove-group-cover-confirm]]",
			function (confirm) {
				if (!confirm) {
					return;
				}
