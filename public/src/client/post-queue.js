...
define("forum/post-queue", [
	...
	function showLinksInPosts() {
		$(".posts-list [data-id]").each((idx, el) => {
			...
			const linkList = linkContainer.find(
				'[component="post-queue/link-container/list"]',
			);
	...
	async function doAction(action, id) {
		...
		const actionsMap = {
			...
			notify: async () =>
				api.post(`/posts/queue/${id}/notify`, { message: await getMessage() }),
		};
	...
	function handleBulkActions() {
		$('[component="post-queue/bulk-actions"]').on(
			"click",
			"[data-action]",
			async function () {
				...
				if (
					bulkAction === "accept-selected" ||
					bulkAction === "reject-selected"
				) {
					queueEls = queueEls.filter((i, el) =>
						$(el).find('input[type="checkbox"]').is(":checked"),
					);
				}
				const ids = queueEls.map((i, el) => $(el).attr("data-id")).get();
				const showConfirm =
					bulkAction === "reject-all" || bulkAction === "reject-selected";
				...
				if (
					...
					(showConfirm &&
						!(await confirmReject(
							`[[post-queue:${translationString}, ${ids.length}]]`,
						)))
				) {
					return;
				}
