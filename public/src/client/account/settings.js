...
define('forum/account/settings', [
	...
	function saveSettings(settings) {
		api
			...
			.then((newSettings) => {
				...
				if (
					languageChanged &&
					parseInt(app.user.uid, 10) === parseInt(ajaxify.data.theirid, 10)
				) {
	...
	function reskin(skinName) {
		const clientEl =
			Array.prototype.filter.call(
				...
				function (el) {
					return (
						el.href.indexOf(config.relative_path + '/assets/client') !== -1
					);
				},
			)[0] || null;
		if (!clientEl) {
	...
	function handleChatAllowDenyList() {
		...
		autocomplete.user($('#chatDenyListAdd'), async function (ev, selected) {
			...
			if (
				$(`[component="chat/deny/list/user"][data-uid="${user.uid}"]`).length
			) {
				return alerts.error('[[error:chat-deny-list-user-already-added]]');
			}
			const html = await app.parseAndTranslate(
				'account/settings',
				'settings.chatDenyListUsers',
				{
		...
		$('[component="chat/allow/list"]').on(
			'click',
			'[component="chat/allow/delete"]',
			function () {
				$(this).parent().remove();
				toggleNoUsersElement();
			},
		...
		$('[component="chat/deny/list"]').on(
			'click',
			'[component="chat/deny/delete"]',
			function () {
				$(this).parent().remove();
				toggleNoUsersElement();
			},
