...
define("forum/chats/pinned-messages", ["api", "alerts"], function (
	api,
	alerts,
) {
	...
	pinnedMessages.init = function (_container) {
		...
		$('[component="chat/pinned/messages/btn"]').on("click", async () => {
			const pinnedMessagesContainer = container.find(
	...
	pinnedMessages.pin = function (mid, roomId) {
		api
			.put(`/chats/${roomId}/messages/${mid}/pin`, {})
			.then(() => {
				$(`[component="chat/message"][data-mid="${mid}"]`).toggleClass(
					"pinned",
					true,
				);
	...
	pinnedMessages.unpin = function (mid, roomId) {
		api
			.del(`/chats/${roomId}/messages/${mid}/pin`, {})
			.then(() => {
				$(`[component="chat/message"][data-mid="${mid}"]`).toggleClass(
					"pinned",
					false,
				...
				container
					.find(`[component="chat/messages/pinned"] [data-mid="${mid}"]`)
					.remove();
				if (
					!container.find(`[component="chat/messages/pinned"] [data-mid]`)
						.length
				) {
					container
						.find('[component="chat/messages/pinned/empty"]')
