...
define('forum/topic', [
	...
	function handleThumbs() {
		...
		$('[component="topic/thumb/list/expand"]').on('click', function () {
			const btn = $(this);
			btn
				.parents('[component="topic/thumb/list"]')
				.removeClass('thumbs-collapsed');
			btn.remove();
		});
	}
	...
	function addParentHandler() {
		...
		components
			.get('topic')
			.on('click', '[component="post/parent"] .timeago', function (e) {
				return gotoPost(
					e,
					$(this).parents('[data-parent-pid]').attr('data-parent-pid'),
				);
			});
	}
			...
			async function () {
				...
				renderTimeout = setTimeout(async () => {
					async function renderPost(pid) {
						const postData =
							postCache[pid] ||
							(await api.get(`/posts/${encodeURIComponent(pid)}/summary`));
						$('#post-tooltip').remove();
						if (postData && ajaxify.data.template.topic) {
							...
							$(window)
								.off('click', onClickOutside)
								.one('click', onClickOutside);
							const css = {
								left: postRect.left,
								width: postWidth,
							};
					...
					const topicMatch =
						validHref && pathname && pathname.match(/\/topic\/([\da-z-]+)/);
					if (postMatch) {
	...
	function addCrosspostsHandler() {
		...
		if (anchorEl) {
			anchorEl.addEventListener('click', async () => {
				...
				const html = await app.parseAndTranslate('modals/crossposts', {
					crossposts,
				});
				bootbox.dialog({
					onEscape: true,
