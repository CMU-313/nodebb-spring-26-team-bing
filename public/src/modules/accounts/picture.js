...
define("accounts/picture", ["pictureCropper", "api", "bootbox", "alerts"], (
	pictureCropper,
	api,
	bootbox,
	alerts,
				...
				app.parseAndTranslate(
					...
					function (html) {
						...
						function updateImages() {
							...
							const iconbgEl = modal.find(
								`[data-bg-color="${ajaxify.data["icon:bgColor"]}"]`,
							);
							if (iconbgEl.length) {
						...
						function saveSelection() {
							const type = modal
								...
								.attr("data-type");
							const iconBgColor =
								modal.find("[data-bg-color].selected").attr("data-bg-color") ||
								"transparent";
							...
							changeUserPicture(type, iconBgColor)
								.then(() => {
									Picture.updateHeader(
										type === "default"
											? ""
											: modal.find(".list-group-item.active img").attr("src"),
										iconBgColor,
									);
									ajaxify.refresh();
								})
	...
	Picture.updateHeader = (picture, iconBgColor) => {
		if (
			parseInt(ajaxify.data.theirid, 10) !== parseInt(ajaxify.data.yourid, 10)
		) {
			return;
		}
		...
		const headerPictureEl = $(
			`[component="header/avatar"] [component="avatar/picture"]`,
		);
		const headerIconEl = $(
	...
	function handleImageUpload(modal) {
		function onUploadComplete(urlOnServer) {
			urlOnServer =
				(!urlOnServer.startsWith("http") ? config.relative_path : "") +
				urlOnServer;
			...
			if (ajaxify.data.picture && ajaxify.data.picture.length) {
				$(
					`#user-current-picture, img[data-uid="${ajaxify.data.theirid}"].avatar`,
		...
		modal.find('[data-action="upload-url"]').on("click", function () {
			modal.modal("hide");
			app.parseAndTranslate(
				"modals/upload-picture-from-url",
				{},
